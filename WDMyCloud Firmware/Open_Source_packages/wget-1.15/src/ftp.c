...
char *transCharset(char *instr, char *fromcode, char *tocode)	// Jacky + 20150401  support multi-language
{
	size_t insize, outsize, conv_res;
	char *inptr, *outbuf, *outptr;
	iconv_t conv_d;

	if (fromcode == NULL || strcmp(fromcode, tocode) == 0 )
		return NULL;
		
	if (tocode == NULL)
		conv_d = iconv_open("UTF-8", fromcode); // convert from fromcode to tocode
	else
		conv_d = iconv_open(tocode, fromcode); // convert from fromcode to tocode
	if(conv_d == (iconv_t)-1)
	{
		return NULL;
	}
...
logprintf (LOG_VERBOSE, "opt.remote_lang='%s', opt.local_lang='%s'\n", opt.remote_lang, opt.local_lang);
  ...
  if ((f == NULL) && (con->target))
    {
      /* Explicit file (like ".listing"). */
			...
			tmp = transCharset(con->target, opt.remote_lang, opt.local_lang);   // Jacky + 20150401  support multi-language 
			if (tmp != NULL) {
				xfree(con->target);
				con->target = tmp;
			}
