...
static int size_buf = WRITEBUFFERSIZE;
static void* buf;

int writeFileInZip(char* filenameinzip, zipFile zf, const char* password, int opt_compress_level, int opt_exclude_path )
{
	FILE *fin = NULL;
	int size_read = 0;
	...
	// skip directory "."
	if (strcmp(filenameinzip, ".") == 0) return ZIP_OK;	

	/* Get information about the file on disk so we can store it in zip */
	filetime(filenameinzip, &zi.tmz_date, &zi.dosDate);

	dir = opendir(filenameinzip);
	if (dir)
	{
		/* Directory exists. */
		closedir(dir);
		...
		// Ensure directory entry has "/" at end of filename
		length=strlen(filenameinzip);
		if (filenameinzip[length-1] != '/')
		{
		    filenameinzip[length] = '/';
		    filenameinzip[length+1] = '\0'; 
		}
		fprintf(stderr, "storing empty directory %s\n", filenameinzip);
	}
	else
	{
	...
	while (savefilenameinzip[0] == '\\' || savefilenameinzip[0] == '/' || (savefilenameinzip[0] == '.' && savefilenameinzip[1] == '/'))
		savefilenameinzip++;
	...
	if (err != ZIP_OK)
		fprintf(stderr,"error in opening %s in zipfile (%d)\n", filenameinzip, err);
	else
	{
		fin = FOPEN_FUNC(filenameinzip, "rb");
		if (fin == NULL)
	...
	if (err == ZIP_OK && !isDir)
	{
		...
		do
		{
			...
			if (size_read > 0)
			{
				...
				if (err < 0)
					fprintf(stderr,"error in writing %s in the zipfile (%d)\n", filenameinzip, err);
			}
		}
		while ((err == ZIP_OK) && (size_read > 0));
	}
	...
	else
	{
		...
		if (err != ZIP_OK)
			fprintf(stderr,"error in closing %s in the zipfile (%d)\n", filenameinzip, err);
	}
	return err;
}
...
void do_help()
{
    printf("Usage : minizip [-o] [-a] [-0 to -9] [-p password] [-j] [-s] [-i] file.zip [files_to_add]\n\n" \
		   ...
		   "  -s  stream to stdout.\n" \
		   "  -i  file list from stdin (delimited by newline char).\n\n");
}
...
int main(int argc, char *argv[])
{
    ...
    if (opt_stdout == 0 && zipfilenamearg == 0)
    {
        do_help();
        return 0;
    }
	...
	else
	{
		...
		else if (opt_overwrite == 0)
		{
			...
			if (check_file_exists(zipfilename) != 0)
			{
				...
				{
					char answer[128];
					fprintf(stderr,"The file %s exists. Overwrite ? [y]es, [n]o, [a]ppend : ", zipfilename);
    ...
    if (zf == NULL)
    {
        ...
        if (opt_stdout == 0) fprintf(stderr,"creating %s\n", zipfilename);

	if (opt_filelist_stdin == 1)
	{
		/* Add files from stdin */
		FILE * fp;
		fp = stdin;
		char path[PATH_MAX];
		while ( (fgets(path, PATH_MAX-1, fp) != NULL) && err==ZIP_OK )
		{
			/* replace ending newline */
			size_t len = strlen(path);
			if (path[len-1] == '\n')
				path[len-1] = '\0';
			err = writeFileInZip(path, zf, password, opt_compress_level, opt_exclude_path);
		}
	}
	else
	{
		...
		for (i = zipfilenamearg + 1; (i < argc) && (err == ZIP_OK); i++)
		{
			...
			err = writeFileInZip(argv[i], zf, password, opt_compress_level, opt_exclude_path);
		}
    }
	
    errclose = zipClose(zf, NULL);
    if (errclose != ZIP_OK)
        fprintf(stderr,"error in closing %s (%d)\n", zipfilename, errclose);
