...
#define EXTENDEDHEADERMAGIC         (0x08074b50)
...
#  define VERSIONMADEBY             (36) /* platform dependent */
//#endif
...
typedef struct
{
	...
	ZPOS64_T cur_pos;               /* current position in file */
...
local int zip64local_putValue OF((const zlib_filefunc64_32_def* pzlib_filefunc_def, voidpf filestream, ZPOS64_T x, int nbByte, ZPOS64_T* cur_pos));
local int zip64local_putValue (const zlib_filefunc64_32_def* pzlib_filefunc_def, voidpf filestream, ZPOS64_T x, int nbByte, ZPOS64_T* cur_pos)
{
    unsigned char buf[8];
    int n;
    for (n = 0; n < nbByte; n++)
    ...
    zlib_filefunc64_32_def* pzlib_filefunc64_32_def)
{
    ...
    ziinit.begin_pos = (ziinit.filestream == stdout) ? 0 : ZTELL64(ziinit.z_filefunc,ziinit.filestream);
    ...
    int strategy, const char* password, uLong crcForCrypting, uLong versionMadeBy, uLong flagBase, int zip64)
{
    ...
    zi->ci.raw = raw;
    zi->ci.flag = flagBase;
	zi->ci.flag |= 8;  // Use extended fields
    if ((level == 8) || (level == 9))
        zi->ci.flag |= 2;
    if (level == 2)
        zi->ci.flag |= 4;
    ...
    if (zi->disk_size > 0)
    {
        if ((zi->number_disk == 0) && (zi->number_entry == 0))
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)DISKHEADERMAGIC, 4, &zi->ci.cur_pos );
    ...
    zi->ci.pos_local_header = (zi->filestream == stdout) ? zi->ci.cur_pos : ZTELL64(zi->z_filefunc,zi->filestream);
    ...
    err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)LOCALHEADERMAGIC, 4, &zi->ci.cur_pos);

    if (err == ZIP_OK)
    {
        if (zi->ci.zip64)
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)45, 2, &zi->ci.cur_pos); /* version needed to extract */
        else
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)20, 2, &zi->ci.cur_pos); /* version needed to extract */
    }
    if (err == ZIP_OK)
        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)zi->ci.flag, 2, &zi->ci.cur_pos);
    if (err == ZIP_OK)
        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)zi->ci.method, 2, &zi->ci.cur_pos);
    if (err == ZIP_OK)
        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)zi->ci.dosDate, 4, &zi->ci.cur_pos);

    /* CRC & compressed size & uncompressed size will be filled in later and rewritten later */

    if (err == ZIP_OK)
        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)0, 4, &zi->ci.cur_pos); /* crc 32, unknown */
    if (err == ZIP_OK)
    {
        if (zi->ci.zip64)
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)0xFFFFFFFF, 4, &zi->ci.cur_pos); /* compressed size, unknown */
        else
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)0, 4, &zi->ci.cur_pos); /* compressed size, unknown */
    }
    if (err == ZIP_OK)
    {
        if (zi->ci.zip64) /* uncompressed size, unknown */
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)0xFFFFFFFF, 4, &zi->ci.cur_pos);
        else /* uncompressed size, unknown */
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)0, 4, &zi->ci.cur_pos);
    }
    if (err == ZIP_OK)
        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)size_filename, 2, &zi->ci.cur_pos);
    if (err == ZIP_OK)
    {
        ...
        if (zi->ci.method == AES_METHOD)
            size_extrafield += 11;
#endif
        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)size_extrafield,2, &zi->ci.cur_pos);
    }
    if ((err == ZIP_OK) && (size_filename > 0))
    {
	    zi->ci.cur_pos += size_filename;
        if (ZWRITE64(zi->z_filefunc, zi->filestream, filename, size_filename) != size_filename)
            err = ZIP_ERRNO;
    }
    if ((err == ZIP_OK) && (size_extrafield_local > 0))
    {
	    zi->ci.cur_pos += size_extrafield_local;
    ...
    if ((err == ZIP_OK) && (zi->ci.zip64))
    {
        ...
        zi->ci.pos_zip64extrainfo = (zi->filestream == stdout) ? zi->ci.cur_pos : ZTELL64(zi->z_filefunc,zi->filestream);

        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (short)headerid, 2, &zi->ci.cur_pos);
        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (short)datasize, 2, &zi->ci.cur_pos);

        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (ZPOS64_T)uncompressed_size, 8, &zi->ci.cur_pos);
        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (ZPOS64_T)compressed_size, 8, &zi->ci.cur_pos);
    }
#ifdef HAVE_AES
    ...
    if ((err == ZIP_OK) && (zi->ci.method == AES_METHOD))
    {
        ...
        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, headerid, 2, &zi->ci.cur_pos);
        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, datasize, 2, &zi->ci.cur_pos);

        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, AES_VERSION, 2, &zi->ci.cur_pos);
        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, 'A', 1, &zi->ci.cur_pos);
        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, 'E', 1, &zi->ci.cur_pos);
        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, AES_ENCRYPTIONMODE, 1, &zi->ci.cur_pos);
        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, zi->ci.compression_method, 2, &zi->ci.cur_pos);
    }
#endif
    ...
    int memLevel, int strategy, const char* password, uLong crcForCrypting)
{
    return zipOpenNewFileInZip4_64(file, filename, zipfi, extrafield_local, size_extrafield_local,
        extrafield_global, size_extrafield_global, comment, method, level, raw, windowBits, memLevel,
        strategy, password, crcForCrypting, VERSIONMADEBY, 1<<11, 0);
}
    ...
    int memLevel, int strategy, const char* password, uLong crcForCrypting, int zip64)
{
    return zipOpenNewFileInZip4_64(file, filename, zipfi, extrafield_local, size_extrafield_local,
        ...
        password, crcForCrypting, VERSIONMADEBY,  1<<11, zip64);
}
...
int Write_ExtendedFileHeader(zip64_internal* zi, uLong crc32, ZPOS64_T compressed_size, ZPOS64_T uncompressed_size  )
{
  /* write the header */
  int err;
 
  err = zip64local_putValue(&zi->z_filefunc,zi->filestream,(uLong)EXTENDEDHEADERMAGIC, 4, &zi->ci.cur_pos);

  // CRC / Compressed size / Uncompressed size
  if (err==ZIP_OK)
	err = zip64local_putValue(&zi->z_filefunc,zi->filestream,crc32,4, &zi->ci.cur_pos); /* crc 32 */
	
  if (err==ZIP_OK)
  {
		if (zi->ci.zip64) 
		{
			err = zip64local_putValue(&zi->z_filefunc, zi->filestream, compressed_size,8, &zi->ci.cur_pos);
			err = zip64local_putValue(&zi->z_filefunc, zi->filestream, uncompressed_size,8, &zi->ci.cur_pos);
		}
		else
		{
			err = zip64local_putValue(&zi->z_filefunc, zi->filestream, compressed_size,4, &zi->ci.cur_pos);
			err = zip64local_putValue(&zi->z_filefunc, zi->filestream, uncompressed_size,4, &zi->ci.cur_pos);
		}
  }
...
extern int ZEXPORT zipCloseFileInZipRaw64(zipFile file, ZPOS64_T uncompressed_size, uLong crc32)
{
    ...
    if (zi->ci.method == AES_METHOD)
    {
		...
		zi->ci.cur_pos += AES_AUTHCODESIZE;
    ...
    if (err == ZIP_OK)
    {
		err = Write_ExtendedFileHeader(zi, crc32, compressed_size, uncompressed_size);
    }
...
extern int ZEXPORT zipClose(zipFile file, const char* global_comment)
{
    ...
    centraldir_pos_inzip = (zi->filestream == stdout) ? zi->ci.cur_pos : ZTELL64(zi->z_filefunc,zi->filestream);
    ...
    if (err == ZIP_OK)
    {
        ...
        while (ldi!= NULL)
        {
            ...
            {
				zi->ci.cur_pos += ldi->filled_in_this_block;
    ...
    {
        ZPOS64_T zip64eocd_pos_inzip = (zi->filestream == stdout) ? zi->ci.cur_pos : ZTELL64(zi->z_filefunc,zi->filestream);
        uLong zip64datasize = 44;

        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)ZIP64ENDHEADERMAGIC, 4, &zi->ci.cur_pos);

        /* size of this 'zip64 end of central directory' */
        if (err == ZIP_OK)
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (ZPOS64_T)zip64datasize, 8, &zi->ci.cur_pos);
        /* version made by */
        if (err == ZIP_OK)
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)45, 2, &zi->ci.cur_pos);
        /* version needed */
        if (err == ZIP_OK)
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)45, 2, &zi->ci.cur_pos);
        /* number of this disk */
        if (err == ZIP_OK)
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)zi->number_disk_with_CD, 4, &zi->ci.cur_pos);
        /* number of the disk with the start of the central directory */
        if (err == ZIP_OK)
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)zi->number_disk_with_CD, 4, &zi->ci.cur_pos);
        /* total number of entries in the central dir on this disk */
        if (err == ZIP_OK)
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, zi->number_entry, 8, &zi->ci.cur_pos);
        /* total number of entries in the central dir */
        if (err == ZIP_OK)
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, zi->number_entry, 8, &zi->ci.cur_pos);
        /* size of the central directory */
        if (err == ZIP_OK)
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (ZPOS64_T)size_centraldir, 8, &zi->ci.cur_pos);

        if (err == ZIP_OK)
        {
            /* offset of start of central directory with respect to the starting disk number */
            ZPOS64_T pos = centraldir_pos_inzip - zi->add_position_when_writting_offset;
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (ZPOS64_T)pos, 8, &zi->ci.cur_pos);
        }
        if (err == ZIP_OK)
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)ZIP64ENDLOCHEADERMAGIC, 4, &zi->ci.cur_pos);
        ...
        if (err == ZIP_OK)
        {
            ...
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, pos, 8, &zi->ci.cur_pos);
        }
        /* number of the disk with the start of the central directory */
        if (err == ZIP_OK)
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)zi->number_disk_with_CD+1, 4, &zi->ci.cur_pos);
    }
    ...
    /* signature */
    if (err == ZIP_OK)
        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)ENDHEADERMAGIC, 4, &zi->ci.cur_pos);
    /* number of this disk */
    if (err == ZIP_OK)
        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)zi->number_disk_with_CD, 2, &zi->ci.cur_pos);
    /* number of the disk with the start of the central directory */
    if (err == ZIP_OK)
        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)zi->number_disk_with_CD, 2, &zi->ci.cur_pos);
    /* total number of entries in the central dir on this disk */
    if (err == ZIP_OK)
    {
        if (zi->number_entry >= 0xffff)
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)0xffff, 2, &zi->ci.cur_pos); /* use value in ZIP64 record */
        else
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)zi->number_entry, 2, &zi->ci.cur_pos);
    }
    /* total number of entries in the central dir */
    if (err == ZIP_OK)
    {
    ...
    /* size of the central directory */
    if (err == ZIP_OK)
        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)size_centraldir,4, &zi->ci.cur_pos);
    /* offset of start of central directory with respect to the starting disk number */
    if (err == ZIP_OK)
    {
        ZPOS64_T pos = centraldir_pos_inzip - zi->add_position_when_writting_offset;
        if (pos >= 0xffffffff)
            ...
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)pos, 4, &zi->ci.cur_pos);
    }
    ...
    if (err == ZIP_OK)
        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)size_global_comment, 2, &zi->ci.cur_pos);
    if (err == ZIP_OK && global_comment > 0)
    {
		zi->ci.cur_pos += size_global_comment;
