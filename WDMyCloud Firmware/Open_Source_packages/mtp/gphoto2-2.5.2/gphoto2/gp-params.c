                          ...
                          float current, void *data)
{
	...
	cur_percent = (int)( ((p->totalRead + (current*100000)) * 100)/p->totalLength );
	if(cur_percent != percent){
		if((p->out_file) && (p->filename != NULL)){
			sprintf (buf,"echo \"%d/%d %d%% %s\" >%s",p->filecount,p->totalfile,cur_percent,p->filename,p->out_file);
			system(buf);	
		}else{
			if(p->filename){
				printf("Downloading %d/%d %d%% %s\n",p->filecount,p->totalfile,cur_percent,p->filename);
			}
		}	
	}
	percent = cur_percent; 
