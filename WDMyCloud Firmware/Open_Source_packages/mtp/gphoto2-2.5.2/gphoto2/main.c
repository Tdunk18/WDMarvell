...
//printf("  %s() %d  s [%s]\n", __func__, __LINE__, s);
//printf("  %s() %d  flags=%x %x\n", __func__, __LINE__, gp_params.flags, gp_params.flags & FLAGS_COPY_KEEP);

        if ((gp_params.flags & FLAGS_COPY_KEEP) != 0)		// alpha.jeffrey20140701  for the same file exists
        {
			fflush (stdout);
			if (gp_system_is_file (s) )	// regular file  (on storage)
			{
#if 0
				struct stat sbuf;
				res = stat(s, &sbuf);
...
//	printf("  size= %ld\n", size);
				CR (gp_file_get_mtime (file, &mtime));
//	printf("  mtime= %ld\n", mtime);
        		if (mtime == 0)
  			      	mtime = 1355313600;		// 2012/12/12 12:0:0 UTC

				if (sbuf.st_size == size && sbuf.st_mtime == mtime)
				{
				...
				//	printf("is different \n");
					...
					fn = malloc(ext - filename +1);
					memset(fn, 0, ext - filename +1);
					strncpy(fn, filename, ext - filename);

					for (idx=1; ; idx++)
					{
						asprintf(&new_filename, "%s_%02d%s", fn, idx, ext);
	//printf("new_filename [%s]\n", new_filename);
						if (access(new_filename, R_OK) == 0)
						{
	//printf("%s file already exist!\n", new_filename);
#if 0
							struct stat sbuf2;
							res = stat(new_filename, &sbuf2);
//	printf("  size= %ld\n", size);
//	printf("  mtime= %ld\n", mtime);
//	printf("  size= %lld\n", sbuf2.st_size);
//	printf("  mtime= %ld\n", sbuf2.st_mtime);
							if (sbuf2.st_size == size && sbuf2.st_mtime == mtime)
							{
				//	printf("the same file [%s] [%s]\n", s, new_filename);
								strcpy(s, new_filename);
								free(new_filename);
								break;
							}
#endif //0
	...
	//printf("=== new_filename [%s]\n", new_filename);
        ...
        if ((gp_params.flags & FLAGS_QUIET) == 0) {
			...
			s[strlen (s) - 1] = 0;
				...
				// 2014/06/18 zoe_sun@alphanetworks.com start
                //printf (_("Saving file as %s\n"), s);
				// 2014/06/18 zoe_sun@alphanetworks.com end
		fflush (stdout);
        }
	path = s;
...
#if 0
        if (mtime == 0 && (gp_params.flags & FLAGS_COPY_KEEP) != 0)	// alpha.jeffrey20140701  for the same file exists
        	mtime = 1355313600;		// 2012/12/12 12:0:0 UTC
#endif //0
        if ((res == GP_OK) && (mtime)) {
                u.actime = mtime;
		   ...
		   CameraFileType type)
{
	...
	if (res < GP_OK) {
		...
		// 2014/09/23 zoe_sun@alphanetworks.com start
		// We let process keep going
		//return res;
		return (GP_OK);
		// 2014/09/23 zoe_sun@alphanetworks.com end
	}
	if(mysave)
		fputs("ok\n",mysave);
...
typedef enum {
	...
	ARG_FOLDER,
	ARG_FORCE_OVERWRITE,
	ARG_COPY_KEEP,	// alpha.jeffrey20140701  for the same file exists
	     ...
	     CallbackParams *params)
{
	...
	case ARG_FORCE_OVERWRITE:
		...
		break;
	case ARG_COPY_KEEP:	// alpha.jeffrey20140701  for the same file exists
		gp_params.flags |= FLAGS_COPY_KEEP;
		break;
	case ARG_NO_FILTER:
		gp_params.flags |= FLAGS_NO_FILTER;
		break;
	case ARG_NEW:
...
#else
		params->p.r = for_each_file (&gp_params, delete_all_action);
#endif
		break;
...
// 2014/06/18 zoe_sun@alphanetworks.com start
static void get_total_lenth(){
	uint64_t totalLength; 
	unsigned int totalfile; 
	
	for_each_file (&gp_params, totalLength_action);	
	totalLength = gp_params.totalLength; 
	totalfile = gp_params.totalfile; 	
	printf ("%d/%d %llu %llu\n",gp_params.filecount, gp_params.totalfile, gp_params.totalRead,gp_params.totalLength);
	gp_params_exit (&gp_params);
	//gp_params_init (&gp_params, envp);
	gp_params.totalLength = totalLength;
	gp_params.totalfile = totalfile;
}
// 2014/06/18 zoe_sun@alphanetworks.com end
...
main (int argc, char **argv, char **envp)
{
	...
	const struct poptOption generalOptions[] = {
		...
		{"quiet", '\0', POPT_ARG_NONE, NULL, ARG_QUIET,
		 ...
		 {"out-file", '\0', POPT_ARG_STRING, (void *) &out_file, ARG_OUT_FILE,
		 N_("Output File"), N_("FILENAME")},
		 {"compare-path", '\0', POPT_ARG_STRING, (void *) &compare_path, ARG_CMP_PATH,
		 N_("Compare Path"), N_("FILENAME")},
	...
	const struct poptOption fileOptions[] = {
		...
		{"no-filter", '\0', POPT_ARG_NONE, NULL, ARG_NO_FILTER,
		 N_("Show all, dependent on list-files"), NULL},
		...
		{"copy-keep", '\0', POPT_ARG_NONE, NULL,
		 ARG_COPY_KEEP, N_("Copy but keep both files"), NULL},	// alpha.jeffrey20140701  for the same file exists
		POPT_TABLEEND
	};
	...
	if(out_file){
		gp_params.out_file = strdup(out_file);
		printf(" out_file=%s\n",(char *)out_file);
	}
	if(compare_path){
		gp_params.compare_path = strdup(compare_path);
		printf(" compare_path=%s\n",(char *)compare_path);
	}
...
#define CHECK_OPT(o)					\
	...
	CHECK_OPT (ARG_OUT_FILE);
	CHECK_OPT (ARG_CMP_PATH);
