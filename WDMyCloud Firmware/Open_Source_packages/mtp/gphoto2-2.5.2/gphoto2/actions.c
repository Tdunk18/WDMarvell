...
#else
delete_all_action (GPParams *p, const char *folder, const char *filename)
{
	CameraFileInfo info;
	int	ret;
	int percent;
...
// only for -P paramater
//--------------------------------------------------------------
int filter(GPParams *p, CameraFileInfo *info, char *filename)
{
	int i, val, do_ignore = 1;
	...
	for(i = 0; i< 1; i++){
		if (*filename == filter_name[0][i]){
			return 0;
		}
	}

	if ((p->flags & FLAGS_NO_FILTER) == 0) {
		if( strcmp(info->file.type, "application/x-unknown") == 0 ) {
			val = sizeof(raw_image_file) / sizeof(char *);
			for(i = 0; i< val; i++) {
				if (!strcasecmp(filename + strlen(filename) - 4, raw_image_file[i])) {
					do_ignore = 0;
					break;
				}
			}
			val = sizeof(media_file) / sizeof(char *);
			for(i = 0; i< val; i++) {
				if (!strcasecmp(filename + strlen(filename) - 4, media_file[i])) {
					do_ignore = 0;
					break;
				}
			}
	...
	if( p->compare_path ){
		...
		path = malloc(strlen(p->compare_path) + strlen(filename) +64);
		if(!path)
			return 1;
		
		sprintf(path, "%s/%s",	p->compare_path, filename );
		...
		if (sbuf.st_size == info->file.size && sbuf.st_mtime == info->file.mtime){
			//printf("same file %s/%s\n", p->compare_path,filename );
			return 0;
		}	
...
int
totalLength_action (GPParams *p, const char *folder, const char *filename)
{
	CameraFileInfo info;
	...
	if (gp_camera_file_get_info (p->camera, folder, filename,
		...
		if( filter(p,&info, filename) ){
			p->totalLength += info.file.size;
			p->totalfile ++;
			if(mytotal){
				char buf[1024];
				sprintf(buf, "[%d] %s\n",p->totalfile,filename);
				fputs(buf,mytotal);
			}			
		}
	}
...
save_file_action (GPParams *p, const char *folder, const char *filename)
{
	...
	if (gp_camera_file_get_info (p->camera, folder, filename,
		...
		if( filter(p,&info, filename) ){
			...
			if(mysave){
				sprintf(buf, "[%d] %s %llu",p->filecount,filename,p->totalRead);
				fputs(buf,mysave);
			}
			ret = save_file_to_file (p->camera, p->context, p->flags,
				   folder, filename, GP_FILE_TYPE_NORMAL);
			p->totalRead += info.file.size;
			// when file size is smaller then CONTEXT_BLOCK_SIZE(100000), it won't update process in gp-param.c
			// so we have to add these part here for small file to count
			percent = (int)( (p->totalRead * 100)/p->totalLength );
			if((p->out_file != NULL) && (p->filename != NULL)){	
				sprintf (buf,"echo \"%d/%d %d%% %s\" >%s",p->filecount,p->totalfile,percent,p->filename,p->out_file);
				system(buf);
			}else{
				if(p->filename){
					printf("Downloading %d/%d %d%% %s\n",p->filecount,p->totalfile,percent,p->filename);
				}
			}	
			return ret;
		}
