...
#define IDSTRSZ 20
static char idstr[IDSTRSZ] = "";
static int min_cpu = 3;
static count_t min_disk_bytes = 512000;
static count_t min_network_bytes = 1000000; 
...
struct pridef { 
	...
	int     print_timestamp; /* print timestamp inside function (allows for filtering)      */
...
atopsar(int argc, char *argv[])
{
	...
	*/
	if (argc > 1)
	{
		...
		** add generic flags
		*/
		strcat(flaglist, "b:e:SxCMHr:q:R:aAX:B:E:");
		...
		while ((c=getopt(argc, argv, flaglist)) != EOF)
		{
			switch (c)
			{
			   ...
			   case 'q':		/* reading of id string (for analytics)? */
				strncpy(idstr, optarg, IDSTRSZ-1);
				rawreadflag++;
				break;
			   case 'X': /* Analytics, min CPU */
			     min_cpu = atoi(optarg);
				 break;
			   case 'B': /* Analytics, min diskIO bytes */
			     min_disk_bytes = atoll(optarg);
			     break;
			   case 'E': /* Analytics, min network bytes */
			     min_network_bytes = atoll(optarg);
			     break;
		...
		*/
		...
		for (i=0; i < pricnt && nr > 0; i++)
		{
			...
			*/
			...
			printf("%s  ", convtime_rfc3339(curtime-numsecs, timebuf));
			...
			*/
		    printf("%s    ", convtime_rfc3339(curtime, timebuf) );
		...
		int nexit, unsigned int noverflow, char flags)
{
	...
	*/
	if (curline >= headline)
	{
		...
		*/
		...
		printf("%s  ", convtime_rfc3339(pretime, timebuf));
	...
	*/
	if (flags & RRBOOT)
	{
		...
		*/
		...
		{
			printf("%s  ", convtime_rfc3339(lasttime, timebuf));
	...
	*/
	if (summarycnt == 1)
	{
		convtime_rfc3339(curtime, timebuf);
		if (pridef[prinow].print_timestamp == 0) 
		{
			printf("%s ", timebuf );
	    }
...
pratopsaruse(char *myname)
{
	...
	fprintf(stderr,
		"\t  -q  id string for Analytics print out\n");
	fprintf(stderr,
		"\t  -X  minimum CPU threshold for Analytics print out\n");
	fprintf(stderr,
		"\t  -B  minimum disk IO byte threshold for Analytics print out\n");
	fprintf(stderr,
		"\t  -E  minimum network byte threshold for Analytics print out\n");
	fprintf(stderr, "\n");
	fprintf(stderr,
...
*/
static void
analyticsEmpty_head(int osvers, int osrel, int ossub)
{
...
static int
analyticsCPU_line(struct sstat *ss, struct tstat *ts, struct tstat **ps, int nactproc,
        time_t deltasec, time_t deltatic, time_t hz,
        int osvers, int osrel, int ossub, char *tstamp,
        int ppres,  int ntrun, int ntslpi, int ntslpu, int pexit, int pzombie)
{
	...
	*/
	...
	{
		double cpu = (double)((ps[i])->cpu.stime + (ps[i])->cpu.utime)*100.0/availcpu;
		char str[500];
		if (i==0 || cpu > (double)min_cpu)
		{
		    int num=i+1;
			total_cpu += cpu;
			sprintf(str, "n%i:%s-%i,c%i:%3.01f%%",num,ps[i]->gen.name,ps[i]->gen.pid,num,cpu); 
			strcat(printbuf, ",");
			strcat(printbuf,str);
		}
		else
	...
	if (total_cpu > (double)min_cpu)
	{
		printf("%s %s ", tstamp, idstr);
		printf("{ mi:cpu,total:%3.01f%%%s }\n", total_cpu, printbuf);
		return 1;
	}
	return 0;
...
static int
analyticsDisk_line(struct sstat *ss, struct tstat *ts, struct tstat **ps, int nactproc,
        time_t deltasec, time_t deltatic, time_t hz,
        int osvers, int osrel, int ossub, char *tstamp,
        int ppres,  int ntrun, int ntslpi, int ntslpu, int pexit, int pzombie)
{
	int		i;
	count_t		tot_read=0, tot_write=0;
    count_t  min_sectors = min_disk_bytes/BYTES_PER_SECTOR;
	if (!ts)
	{
		printf("report not available.....\n");
		return 0;
	...
	/*
	** determine total disk io for all processes
	*/
	for (i=0; i < nactproc; i++)
	{
		tot_read += (ps[i])->dsk.rsz;
		tot_write += (ps[i])->dsk.wsz;
	}
	...
	*/
	...
	if (tot_read > min_sectors || tot_write > min_sectors )
	{
		printf("%s %s ", tstamp, idstr);

		// display in MB
		printf("{ mi:io,r_MB:%3.02f,w_MB:%3.02f", (double)tot_read * BYTES_PER_SECTOR/1000000, (double)tot_write * BYTES_PER_SECTOR/1000000);
		for (i=0;i < TOP_PROCESSES; i++)
		{
			int num=i+1;
		    if (i==0 || (ps[i])->dsk.rsz > min_sectors || (ps[i])->dsk.wsz > min_sectors )
			{
				printf(",n%i:%s-%i,r%i_MB:%3.02f,w%i_MB:%3.02f",num,ps[i]->gen.name,ps[i]->gen.pid,
					num, (double)(ps[i])->dsk.rsz * BYTES_PER_SECTOR/1000000,
					num, (double)(ps[i])->dsk.wsz * BYTES_PER_SECTOR/1000000);
			}
		}
		printf(" }\n");
		return 1;
...
static int
analyticsNetwork_line(struct sstat *ss, struct tstat *ts, struct tstat **ps, int nactproc,
        time_t deltasec, time_t deltatic, time_t hz,
        int osvers, int osrel, int ossub, char *tstamp,
        int ppres,  int ntrun, int ntslpi, int ntslpu, int pexit, int pzombie)
{
	int		i;
	count_t	total_rx, total_tx;
	...
	** determine total network accesses for all processes
	*/
	for (i=0,total_rx=0,total_tx=0; i < nactproc; i++)
	{
		total_rx += (ps[i])->net.tcprsz + (ps[i])->net.udprsz;
		total_tx += (ps[i])->net.tcpssz + (ps[i])->net.udpssz;
	}
	...
	*/
	...
	if (total_rx > min_network_bytes || total_tx > min_network_bytes )
	{
		printf("%s %s ", tstamp, idstr);
		// display in MB
		printf("{ mi:network,rx_MB:%3.02f,tx_MB:%3.02f", (double)total_rx/1000000, (double)total_tx/1000000);
		for (i=0;i < TOP_PROCESSES; i++)
		{
			int num=i+1;
			count_t rx_bytes = (ps[i])->net.tcprsz + (ps[i])->net.udprsz;
			count_t tx_bytes = (ps[i])->net.tcpssz + (ps[i])->net.udpssz;
		    if (i==0 || rx_bytes > min_network_bytes || tx_bytes > min_network_bytes )
			{
				printf(",n%irx:%s-%i,rx%i_MB:%3.02f,tx%i_MB:%3.02f",num,ps[i]->gen.name,ps[i]->gen.pid,
					num, (double)rx_bytes/1000000,
					num, (double)tx_bytes/1000000);
			}
		}
		printf(" }\n");
		return 1;
...
static int
analyticsMemory_line(struct sstat *ss, struct tstat *ts, struct tstat **ps, int nactproc,
        time_t deltasec, time_t deltatic, time_t hz,
        int osvers, int osrel, int ossub, char *tstamp,
        int ppres,  int ntrun, int ntslpi, int ntslpu, int pexit, int pzombie)
{
	...
	*/
	...
	printf("%s %s ", tstamp, idstr);
	// display in MB
	printf("{ mi:mem,tot:%lld,buf:%lld,cache:%lld,swp:%lld",
		(ss->mem.physmem-ss->mem.freemem) * (pagesize / 1024) /1024,
		ss->mem.buffermem * (pagesize / 1024) /1024,
		ss->mem.cachemem  * (pagesize / 1024) /1024,
		(ss->mem.totswap-ss->mem.freeswap) * (pagesize / 1024) /1024
	);
	for (i=0;i < TOP_PROCESSES; i++)
	{
		int num=i+1;
		printf(",n%i:%s-%i,KB%i:%lld,pf%i:%lld",num,ps[i]->gen.name,ps[i]->gen.pid,num, ps[i]->mem.rmem, num, ps[i]->mem.majflt);
	}
...
static int
analytics_line(struct sstat *ss, struct tstat *ts, struct tstat **ps, int nactproc,
        time_t deltasec, time_t deltatic, time_t hz,
        int osvers, int osrel, int ossub, char *tstamp,
        int ppres,  int ntrun, int ntslpi, int ntslpu, int pexit, int pzombie)
{

	int print_cpu = analyticsCPU_line(ss,ts,ps,nactproc,deltasec,deltatic,hz,osvers,osrel,ossub,tstamp,ppres,ntrun,ntslpi,ntslpu,pexit,pzombie);
	int print_disk = analyticsDisk_line(ss,ts,ps,nactproc,deltasec,deltatic,hz,osvers,osrel,ossub,tstamp,ppres,ntrun,ntslpi,ntslpu,pexit,pzombie);
	int print_network = analyticsNetwork_line(ss,ts,ps,nactproc,deltasec,deltatic,hz,osvers,osrel,ossub,tstamp,ppres,ntrun,ntslpi,ntslpu,pexit,pzombie);
	if (print_cpu || print_disk || print_network)
	{
		analyticsMemory_line(ss,ts,ps,nactproc,deltasec,deltatic,hz,osvers,osrel,ossub,tstamp,ppres,ntrun,ntslpi,ntslpu,pexit,pzombie);
	}
	return 1;
}
...
{
   {0,  "c",  'c',  0, cpuhead,	cpuline,  	"cpu utilization",        },
   {0,  "c",  'p',  0,prochead,	procline,  	"process(or) load",       },
   {0,  "c",  'P',  0,taskhead,	taskline,  	"processes & threads",    },
   {0,  "m",  'm',  0,memhead,	memline,	"memory & swapspace",     },
   {0,  "m",  's',  0,swaphead,	swapline,	"swap rate",              },
   {0,  "cd", 'l',  0,lvmhead,	lvmline,	"logical volume activity", },
   {0,  "cd", 'f',  0,mddhead,	mddline,	"multiple device activity",},
   {0,  "cd", 'd',  0,dskhead,	dskline,	"disk activity",          },
   {0,  "n",  'n',  0,nfmhead,	nfmline,	"NFS client mounts",      },
   {0,  "n",  'j',  0,nfchead,	nfcline,	"NFS client activity",    },
   {0,  "n",  'J',  0,nfshead,	nfsline,	"NFS server activity",    },
   {0,  "n",  'i',  0,ifhead,	ifline,		"net-interf (general)",   },
   {0,  "n",  'I',  0,IFhead,	IFline,		"net-interf (errors)",    },
   {0,  "n",  'w',  0,ipv4head,	ipv4line,	"ip   v4    (general)",   },
   {0,  "n",  'W',  0,IPv4head,	IPv4line,	"ip   v4    (errors)",    },
   {0,  "n",  'y',  0,icmpv4head,	icmpv4line,	"icmp v4    (general)",   },
   {0,  "n",  'Y',  0,ICMPv4head,	ICMPv4line,	"icmp v4    (per type)",  },
   {0,  "n",  'u',  0,udpv4head,	udpv4line,  	"udp  v4",                },
   {0,  "n",  'z',  0,ipv6head,	ipv6line,	"ip   v6    (general)",   },
   {0,  "n",  'Z',  0,IPv6head,	IPv6line,	"ip   v6    (errors)",    },
   {0,  "n",  'k',  0,icmpv6head,	icmpv6line,	"icmp v6    (general)",   },
   {0,  "n",  'K',  0,ICMPv6head,	ICMPv6line,	"icmp v6    (per type)",  },
   {0,  "n",  'U',  0,udpv6head,	udpv6line,  	"udp  v6",                },
   {0,  "n",  't',  0,tcphead,	tcpline,  	"tcp        (general)",   },
   {0,  "n",  'T',  0,TCPhead,	TCPline,  	"tcp        (errors)",    },
#if	HTTPSTATS
   {0,  "n",  'h',  0,httphead,	httpline,  	"HTTP activity",          },
#endif
   {0,  "",   'O',  0,topchead,	topcline,  	"top-3 processes cpu",    },
   {0,  "",   'G',  0,topmhead,	topmline,  	"top-3 processes memory", },
   {0,  "",   'D',  0,topdhead,	topdline,  	"top-3 processes disk",   },
   {0,  "",   'N',  0,topnhead,	topnline,  	"top-3 processes network",},
   {0,  "",   'Q',  1,analyticsEmpty_head,	analytics_line, "Analytics",},
};
