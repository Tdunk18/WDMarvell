...
static int write_log = 0 , ups_shutflag = 0 , lb_happen_time = 0;
char tsc[2][16] = {"battery.charge","ups.status"};

/*#if defined(LIGHTNING_4A)
int send_to_up = 0 ;
int now_battery_status = 0 ; 
#endif*/
...
static void mainloop(void)
{
	...
	FILE *fp;
	struct stat sta ;
	int lb_now_time = 0 , w_filecheck = 0 , count ;
	const char *upsStatus ;
	//const char *upsBattCharge ;
	char bufs[128];
	...
	for (i = 0; i < nfds; i++) {
		...
		if (fds[i].revents & POLLIN) {
			...
			case DRIVER:
				...
				if (write_log == 1) {
					if ((upsStatus = sstate_getinfo((upstype_t *)handler[i].data, tsc[0])) != NULL) {
/*#if defined(LIGHTNING_4A)		// move to upsmon.c	+ 20141014 Jacky Chang
						upsStatus = sstate_getinfo((upstype_t *)handler[i].data, tsc[1]);
						if (strstr(upsStatus , "OL")) {
							sprintf(bufs , "ups_info -b \"%s\" -s \"OL\"",sstate_getinfo((upstype_t *)handler[i].data, tsc[0]));	
						}
						else if (strstr(upsStatus , "LB")) {
							sprintf(bufs , "ups_info -b \"%s\" -s \"LB\"",sstate_getinfo((upstype_t *)handler[i].data, tsc[0]));		
						}
						else if (strstr(upsStatus , "OB")) {
							sprintf(bufs , "ups_info -b \"%s\" -s \"OB\"",sstate_getinfo((upstype_t *)handler[i].data, tsc[0]));		
						}
						system(bufs);
#else	
//	for D-Link		
						fp = fopen("/tmp/ups_state" , "w");
						for (count = 0 ; count < 2 ; count ++) {
							upsStatus = sstate_getinfo((upstype_t *)handler[i].data, tsc[count]);
...
*/
					...
					else if ((upsStatus = sstate_getinfo((upstype_t *)handler[i].data,tsc[1])) != NULL) {
/*#if defined(LIGHTNING_4A)			// move to upsmon.c
						upsStatus = sstate_getinfo((upstype_t *)handler[i].data, tsc[1]);
						if (strstr(upsStatus , "OL")) {
							sprintf(bufs , "ups_info -b \"N/A\" -s \"OL\"");	
						}
						else if (strstr(upsStatus , "LB")) {
							sprintf(bufs , "ups_info -b \"N/A\" -s \"LB\"");		
						}
						else if (strstr(upsStatus , "OB")) {
							sprintf(bufs , "ups_info -b \"N/A\" -s \"OB\"");		
						}
				...
				if ((upsStatus = sstate_getinfo((upstype_t *)handler[i].data , "ups.status")) != NULL) {
					if (strstr(upsStatus , "OL")) {
						lb_happen_time = 0 ;
						lb_now_time = 0 ;
						if ((w_filecheck = stat("/tmp/UPS_OB",&sta)) >= 0) {
							unlink("/tmp/UPS_OB");
							//#if defined(LIGHTNING_4A)
							//fireAlert(0, 31 , NULL);
							//system("up_send_ctl ErrStatus 0031 0");
							//#endif
						}
					}
					
					if (strstr(upsStatus , "OB")) {
						if ((w_filecheck = stat("/tmp/UPS_OB" , &sta)) < 0) {
							system("touch /tmp/UPS_OB");
						}
						/*
						#if defined(LIGHTNING_4A)
						if ((upsBattCharge = sstate_getinfo((upstype_t *)handler[i].data, tsc[0])) != NULL) {
							if (send_to_up == 0) {
								send_to_up = 1 ;
								memset(bufs , 0 , sizeof(bufs));
								strncpy(bufs , upsBattCharge , strlen(upsBattCharge));
								fireAlert(1, 31 , bufs);
								//sprintf(bufs , "up_send_ctl ErrStatus 0031 1 %s",upsBattCharge);
								//system(bufs);
								now_battery_status = atoi(upsBattCharge);
							}
							else {
								if (now_battery_status != atoi(upsBattCharge)) {
									memset(bufs , 0 , sizeof(bufs));
									strncpy(bufs , upsBattCharge , strlen(upsBattCharge));
									fireAlert(1, 31 , bufs);
									//memset(bufs , 0 , sizeof(bufs));
					...
					if(strstr(upsStatus,"LB")) {
						if(lb_happen_time == 0){
							lb_happen_time = time((time_t*)NULL);
						}
						
						lb_now_time = time((time_t*)NULL);
						
						if (lb_happen_time != 0 && ((lb_now_time - lb_happen_time) >= 30)) {
							if((w_filecheck = stat("/tmp/UPS_LB",&sta)) < 0) {
								system("touch /tmp/UPS_LB");
							}
							ups_shutflag = 1;
						}
					}
...
static void write_infolog(int log)
{
	write_log = 1;
}
...
static void setup_signals(void)
{
	...
	//sa.sa_handler = shuttime;//for_ups_ob_shut
	//sigaction(SIGUSR1, &sa, NULL);//for_ups_ob_shut
	
	sa.sa_handler = write_infolog;
...
int main(int argc, char **argv)
{
	...
	printf("Network UPS Tools - upsd.\n");

	while ((i = getopt(argc, argv, "+h46p:qr:i:fu:X:LVc:D")) != -1) {
		switch (i) {
			case 'h':
				help(progname);
				break;
