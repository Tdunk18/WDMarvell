...
static void recalc(void)
{
	...
	/*
	 * Alpha.tim modify : to suit with our design , let it go ....
	 * */
#if 0
...
static void parse_status(utype_t *ups, char *status, char *battCharge)
{
	char	*statword, *ptr, *upsStatus;
	int		intBattCharge;
	//static int		intBattCharge=100;
	static int	showPowerLeftFlag_50 = ALERT_ON, showPowerLeftFlag_15 = ALERT_ON, showOnBatteryFlag = ALERT_ON, showLowBatteryFlag = ALERT_ON;
	...
	while (statword != NULL) {
		...
		if (!strcasecmp(statword, "OB"))
			ups_on_batt(ups);
		if (!strcasecmp(statword, "LB") && !strstr(status, "OL"))
			ups_low_batt(ups);
	...
	// for USB device info. GUI  + 20141014 Jacky Chang
	system("ups_info");

	
	/*** UPS battery charge alert ***/
	/*
	printf_out("showLowBatteryFlag: '%d'\n", showLowBatteryFlag);
	printf_out("showPowerLeftFlag_15: '%d'\n", showPowerLeftFlag_15);
	printf_out("showPowerLeftFlag_50: '%d'\n", showPowerLeftFlag_50);
	printf_out("upsStatus: %s\n", upsStatus);
	printf_out("battCharge: %s\n", battCharge);
	printf_out("intBattCharge: %d\n", intBattCharge);
	*/
	if (battCharge[0] != 0) {
		//printf_out("Entry...\n");
	  intBattCharge = atoi(battCharge);
	  	
    if (strstr(upsStatus , "OL")) {					// for OL
    	// reset flag
    	//printf_out("reset flag.\n");
	  	showPowerLeftFlag_50 = ALERT_ON;
	  	showPowerLeftFlag_15 = ALERT_ON;
	  	showOnBatteryFlag = ALERT_ON;
	  	showLowBatteryFlag = ALERT_ON;
	  	system("led power blue on");	// power LED Blue light
    } else if (strstr(upsStatus , "OB")) {	// for OB
    	//printf_out("On Battery.\n");
    	system("led power red on");		// power LED RED light
    	if (showOnBatteryFlag == ALERT_ON)	{
    		//fireAlert(0, 31, NULL);						// Alert Code 31: On UPS Power.
    		fireAlert(FIRE_ALERT_CUSTOM_ID, ON_UPS_POWER_ID, battCharge);						// Alert Code 31: On UPS Power.
    		showOnBatteryFlag = ALERT_OFF;
    	}
    	
    	if (intBattCharge <= 50 && showPowerLeftFlag_50 == ALERT_ON) {
    		//printf_out("Show Power Left flag 50.\n");
    		//fireAlert(0, 42, NULL);						// Alert Code 42: 50% UPS Power Left.
    		fireAlert(FIRE_ALERT_CUSTOM_ID, UPS_POWER_FIFTY_PRECENT_ID, NULL);						// Alert Code 42: 50% UPS Power Left.
    		showPowerLeftFlag_50 = ALERT_OFF;
    	}
    		
    	if (intBattCharge <= 15 && showPowerLeftFlag_15 == ALERT_ON)	{
    		//printf_out("Show Power Left flag 15.\n");
    		//fireAlert(0, 43, NULL);						// Alert Code 43: 15% UPS Power Left.
    		fireAlert(FIRE_ALERT_CUSTOM_ID, UPS_POWER_FIFTEEN_PRECENT_ID, NULL);						// Alert Code 43: 15% UPS Power Left.
    		showPowerLeftFlag_15 = ALERT_OFF;
    	}
    } else if(strstr(upsStatus,"LB")) {			// for LB
    	// function move to upssched-cmd
    	/*if (showLowBatteryFlag == ALERT_ON) {
 	  		fireAlert(FIRE_ALERT_CUSTOM_ID, UPS_POWER_OUT_OF_POWER_ID, NULL);						// Alert Code 44: UPS Out of Power.
 	  		showLowBatteryFlag = ALERT_OFF;
 	  	}*/
    }
	}
}
...
static void pollups(utype_t *ups)
{
	...
	get_var(ups, "battCharge", battCharge, sizeof(battCharge));
	//printf_out("Battery charge: '%s'\n", battCharge);

	// get ups.status info.
	if (get_var(ups, "status", status, sizeof(status)) == 0) {
		clear_alarm();
		parse_status(ups, status, battCharge);
		return;
	}
...
int main(int argc, char *argv[])  
{
	...
	printf("Network UPS Tools - upsmon.\n");
