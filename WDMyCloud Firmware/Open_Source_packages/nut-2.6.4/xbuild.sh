...
xbuild()
{
	...
	if [ ! -e ${PWD}/../${openssl_version}/xinst/usr/lib/libssl.so ]; then
		echo ""
		echo -e "\033[31m *ERROR*: You need to build ${openssl_version} first. \033[0m"
		echo ""
	...
	CFLAGS="-I${GPL_PREFIX}/include -I${PWD}/../${openssl_version}/xinst/usr/include -I${XINC_DIR} ${CFLAGS}" LDFLAGS="-s -L${GPL_PREFIX}/lib -L${PWD}/../${openssl_version}/xinst/usr/lib -L${PWD}/../zlib-1.2.3 -lssl -lcrypto -lz -L${XLIB_DIR} -lalert -lxml2 -lxmldbc" ./configure --host=${TARGET_HOST} --prefix="/usr/local/ups" --enable-static=no --with-ssl=yes --with-ipv6=no --with-snmp=no --with-usb=yes --with-serial=no --with-user=root --with-group=root --with-hal=no --enable-strip=yes --with-lib=yes --with-ssl-includes="-I${PWD}/../${openssl_version}/xinst/usr/include" --with-ssl-libs="-L${PWD}/../${openssl_version}/xinst/usr/lib" --with-usb-includes="-I${GPL_PREFIX}/include"
	#else
