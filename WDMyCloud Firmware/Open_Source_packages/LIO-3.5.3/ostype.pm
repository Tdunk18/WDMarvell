sub ostype
{
    ...
    elsif (lc($txt) =~/debian|risingtide/)
    {
    	$rval->{DISTRO}="DEBIAN";
	$rval->{RPM_DIR}="/usr/src/redhat";
	$rval->{OSTYPE}="LINUX";
    ...
    if ($my_arch eq ALPHA) #cyber
    {
		my $LINUX_SRC = "$ENV{'LINUX_SRC'}"; # odie
        $rval->{ARCH}="arm";
        $rval->{DISTRO}="MARVELL-LINUX";
        $rval->{SYSTEM}="MARVELL-LINUX";
        $rval->{KERNEL_DIR}="$LINUX_SRC";
        $rval->{KERNEL_SOURCE_DIR}="$LINUX_SRC";
        $rval->{KERNEL_INCLUDE_DIR}="$LINUX_SRC/include";
        $rval->{OSTYPE}="LINUX";
	...
	$rval->{RELEASE} = "2.6.31.9-Marvell";	# odie
	@{$rval->{RELEASES}} = (); 
	push @{$rval->{RELEASES}}, $rval->{RELEASE};
	$rval->{NO_RPM}=1;
    }
