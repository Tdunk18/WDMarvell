...
void core_tpg_add_node_to_devs(
	...
	se_portal_group_t *tpg)
{
	se_node_acl_t *default_acl;
	int i = 0;
	u32 lun_access = 0;
	se_lun_t *lun;
	
	default_acl = tpg->se_tpg_default_acl;
	if(default_acl) {
...
void core_tpg_copy_node_to_devs(
	se_node_acl_t *dst_acl,
	se_node_acl_t *src_acl,
	se_portal_group_t *tpg)
{
	int i = 0, enable = 1;
	u32 lun_access = 0;
	...
	for (i = 0; i < TRANSPORT_MAX_LUNS_PER_TPG; i++) {
		...
		printk(KERN_INFO "TARGET_CORE[%s]->TPG[%u]_LUN[%u->%u] - Adding %s"
			...
			TPG_TFO(tpg)->get_fabric_name(),
			TPG_TFO(tpg)->tpg_get_tag(tpg), lun->unpacked_lun,mapped_lun,
			(lun_access == TRANSPORT_LUNFLAGS_READ_WRITE) ?
			"READ-WRITE" : "READ-ONLY");

		core_update_device_list_for_node(lun, NULL, mapped_lun,
				lun_access, dst_acl, tpg, enable);
		spin_lock(&tpg->tpg_lun_lock);
	}
	spin_unlock(&tpg->tpg_lun_lock);
}
	...
	unsigned char *initiatorname)
{
	...
	if(TPG_TFO(tpg)->copy_node_attributes) {
		TPG_TFO(tpg)->copy_node_attributes(acl,default_acl);
	}
	else {
	...
	printk("%s_TPG[%u] - Added Default Policy ACL with TCQ Depth: %d for %s"
		" Initiator Node: %s\n", TPG_TFO(tpg)->get_fabric_name(),
