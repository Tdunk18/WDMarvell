...
#ifdef ALPHA_CUSTOMIZE //2012-01-10 - port and modify naa bug fix from LIO v4
	...
	void *transport_dev)
{
	...
	if (TRANSPORT(dev)->get_blocks(dev) > 0x00000000ffffffff)
...
int transport_generic_activate_device(se_device_t *dev)
{
	...
	memset(name, 0, 16);
#ifdef ALPHA_CUSTOMIZE    /* 2012-03-01 Tim Tsay - change vendor name */
	snprintf(name, 16, "WD_%s", TRANSPORT(dev)->name);
#else
...
/* hex_to_bin port from lib/hexdump.c in kernel 3.2 */
/** 
...
static void
target_parse_naa_6h_vendor_specific(const unsigned char *p, unsigned char *buf)
{
	int cnt;
	bool next = true;
	/*
...
#ifdef ALPHA_CUSTOMIZE	/*2012-02-24 Tim Tsay - return check condition if page code != 0 
						    , port from kernel 3.2, see SPC3r23 section 6.4.1*/
		if (cdb[2]) {
			printk(KERN_ERR "INQUIRY with EVPD==0 but PAGE CODE=%02x\n",
			       cdb[2]);
			/* the default error return will set sense code to 
			 * INVALID CDB FIELD in fd_emulate_scsi_cdb */
			return -1;
		}
...
#ifdef ALPHA_CUSTOMIZE	//2012-02-24 Tim Tsay - set RESPONSE DATA FORMAT = 2, port from kernel 3.2
		/*
...
after_tpgs:
		...
		snprintf((unsigned char *)&buf[8], 8, "WD ");
...
#ifdef ALPHA_CUSTOMIZE /* 2012-03-07 Tim Tsay - force transfer length match data length */
		if ((buf[4]+ 5) < cmd->data_length) {
			cmd->se_cmd_flags |= SCF_UNDERFLOW_BIT;
			cmd->residual_count = (cmd->data_length - (buf[4]+ 5));
		}
		else if((buf[4]+ 5) > cmd->data_length ) {
			cmd->se_cmd_flags |= SCF_OVERFLOW_BIT;
			cmd->residual_count = (buf[4]+ 5 - cmd->data_length);
		}
		
		cmd->data_length = buf[4] + 5;
#endif /* endof ALPHA_CUSTOMIZE */
		return 0;
	}
	/*
	...
	case 0x80: /* unit serial number */
		...
		if((len + 4) < cmd->data_length ) {
			cmd->se_cmd_flags |= SCF_UNDERFLOW_BIT;
			cmd->residual_count = (cmd->data_length - (len + 4));
		} else if ((len + 4) > cmd->data_length){
			cmd->se_cmd_flags |= SCF_OVERFLOW_BIT;
			cmd->residual_count = (len + 4 - cmd->data_length);
		}
		
		cmd->data_length = len + 4;
#endif /* endof ALPHA_CUSTOMIZE */
		break;
	case 0x83:
		/*
		 ...
		 */
		target_parse_naa_6h_vendor_specific(&DEV_T10_WWN(dev)->unit_serial[0], &buf[off]);
...
check_t10_vend_desc:
		...
		memcpy((unsigned char *)&buf[off+4], "WD ", 8);
#else
...
#ifdef ALPHA_CUSTOMIZE /* 2012-03-05 Tim Tsay - Support Informational Exception Control Mode Page
 	 	 	 	 	 	  	  	  	  	  	  ref: SPC3r23  section 7.4.11*/
static int transport_modesense_info_except_control(
		unsigned char *p)
{
	p[0] = 0x1c;
	p[1] = 0x0a;
	/* Set disable exception control (DExpect) = 1 */
	p[2] = 0x08;
	...
	int type)
{
	...
	case 0x0a:
		...
		break;
 	 	 	 	 	 	  	  	  	  	  	  ...
 	 	 	 	 	 	  	  	  	  	  	  ref: SPC3r23 section 7.4.11*/
	case 0x1c:
		length = transport_modesense_info_except_control(&buf[offset]);
		break;
#endif /* endof ALPHA_CUSTOMIZE */
...
#ifdef ALPHA_CUSTOMIZE /* 2012-03-05 Tim Tsay - Support Infomational Exception Control Mode Page */
		length += transport_modesense_info_except_control(&buf[offset+length]);
#endif /* endof ALPHA_CUSTOMIZE */
	...
	if (ten) {
		...
		if((offset + 2) < cmd->data_length ) {
			cmd->se_cmd_flags |= SCF_UNDERFLOW_BIT;
			cmd->residual_count = (cmd->data_length - (offset + 2));
			cmd->data_length = offset + 2;
			
		} else if((offset + 2) > cmd->data_length ) {
			cmd->se_cmd_flags |= SCF_OVERFLOW_BIT;
			cmd->residual_count = (offset + 2 - cmd->data_length);
		...
		if((offset + 1) < cmd->data_length ) {
			cmd->se_cmd_flags |= SCF_UNDERFLOW_BIT;
			cmd->residual_count = (cmd->data_length - (offset + 1));
			cmd->data_length = offset + 1;
			
		} else if((offset + 1) > cmd->data_length ) {
			cmd->se_cmd_flags |= SCF_OVERFLOW_BIT;
			cmd->residual_count = (offset + 1 - cmd->data_length);
			offset = cmd->data_length;
		}
...
#ifdef ALPHA_CUSTOMIZE /* 2012-03-07 - Tim Tsay - set sense key of invalid cdb field = illegal request */
		/* ILLEGAL REQUEST */
		buffer[offset+SPC_SENSE_KEY_OFFSET] = ILLEGAL_REQUEST;
		
#else /* !invalid field in cdb */
		/* ABORTED COMMAND */
		buffer[offset+SPC_SENSE_KEY_OFFSET] = ABORTED_COMMAND;
#endif /* endof ALPHA_CUSTOMIZE */
