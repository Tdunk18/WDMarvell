	...
	se_portal_group_t *tpg)
{
	...
	printk("Release device_list from initiator %s\n",nacl->initiatorname);
	...
	u32 lun_access)
{
	 ...
	 */
	if (TPG_TFO(tpg)->tpg_check_demo_mode(tpg) && tpg->se_tpg_default_acl != NULL) { 
		if(!strcmp(tpg->se_tpg_default_acl->initiatorname, nacl->initiatorname)) {
			spin_lock_bh(&tpg->acl_node_lock);
			list_for_each_entry(acl, &tpg->acl_node_list, acl_list) {
				if ((acl->nodeacl_flags & NAF_DYNAMIC_NODE_ACL) && nacl != acl) {
					spin_unlock_bh(&tpg->acl_node_lock);
					core_tpg_add_node_to_devs(acl, tpg);
					spin_lock_bh(&tpg->acl_node_lock);
				}
		...
		if(!strcmp(tpg->se_tpg_default_acl->initiatorname, nacl->initiatorname)) {
			...
			list_for_each_entry(acl, &tpg->acl_node_list, acl_list) {
				if ((acl->nodeacl_flags & NAF_DYNAMIC_NODE_ACL) && nacl != acl) {
					...
					core_update_device_list_for_node(lun, NULL, lacl->mapped_lun,
								TRANSPORT_LUNFLAGS_NO_ACCESS, acl, tpg, 0);
					spin_lock_bh(&tpg->acl_node_lock);
				}
			}
			spin_unlock_bh(&tpg->acl_node_lock);
