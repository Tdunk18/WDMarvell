 ...
 *	Should be called with scsi_device_get(sd) held
 */
int pscsi_claim_sd(pscsi_dev_virt_t *pdv, struct scsi_device *sd)
{
 ...
 * 	Should be called with scsi_device_get(sd) held
 */
int pscsi_release_sd(pscsi_dev_virt_t *pdv, struct scsi_device *sd)
{
	struct gendisk *disk;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30)
        struct scsi_disk *sdisk;
...
int pscsi_release_phydevice(se_device_t *dev)
{
	...
	return pscsi_release_sd(pdv, sd);
}
...
sector_t pscsi_get_blocks(se_device_t *dev)
{
	pscsi_dev_virt_t *pdv = dev->dev_ptr;
