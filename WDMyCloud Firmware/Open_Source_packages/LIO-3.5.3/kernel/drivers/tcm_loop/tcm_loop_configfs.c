	...
	struct tcm_loop_tpg *tpg)
{
	...
	printk(KERN_INFO "TCM_Loop_ConfigFS: Removing I_T Nexus to SAS"
		" Initiator Port: %s\n",
...
CONFIGFS_EATTR_STRUCT(tcm_loop_tpg, se_portal_group_s);
#define TL_TPG_ATTR(_name, _mode)					\
static struct tcm_loop_tpg_attribute tcm_loop_tpg_##_name =		\
	__CONFIGFS_EATTR(_name, _mode,					\
	tcm_loop_tpg_show_##_name,					\
	tcm_loop_tpg_store_##_name);
	...
	size_t count)
{
	struct tcm_loop_tpg *tl_tpg =
			(struct tcm_loop_tpg *) se_tpg->se_tpg_fabric_ptr;
	unsigned char i_port[TL_NAA_SAS_ADDR_LEN], *ptr, *port_ptr;
	int ret;
	/*
	...
	if (!(ptr)) {
		printk(KERN_ERR "Unable to locate prefix for SAS Initiator"
				" WWPN: %s\n", i_port);
		return -EINVAL;
	}
	/*
...
TL_TPG_ATTR(nexus, S_IRUGO | S_IWUSR);

CONFIGFS_EATTR_OPS(tcm_loop_tpg, se_portal_group_s, tpg_group);
...
static struct configfs_item_operations tcm_loop_tpg_item_ops = {
	.show_attribute		= &tcm_loop_tpg_attr_show,
        .store_attribute	= &tcm_loop_tpg_attr_store,
};

static struct config_item_type tcm_loop_tpg_cit = {
	.ct_item_ops		= &tcm_loop_tpg_item_ops,
	.ct_group_ops		= NULL,
	.ct_attrs		= tcm_loop_tpg_attrs,
	.ct_owner		= THIS_MODULE,
};
	...
	const char *name)
{
	...
	tpg_cg->default_groups = kzalloc(sizeof(struct config_group) * 2,
			GFP_KERNEL);
	...
	char *page)
{
	return sprintf(page, "TCM Loopback Fabric module %s on %s/%s"
		" on "UTS_RELEASE"\n", TCM_LOOP_VERSION, TCM_UTS_SYSNAME,
		TCM_UTS_MACHINE);
}
