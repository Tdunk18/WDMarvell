...
#ifdef ALPHA_CUSTOMIZE	//2012-01-03 Tim Tsay - support IPv6 link local address
#include <net/ipv6.h>
#endif
        ...
        const char *name)
{
...
#ifdef ALPHA_CUSTOMIZE		//2012-01-03 Tim Tsay - support IPv6 link local address
	//a given network string in ipv6 link local address should be like [fe80::1%eth0]:3260 
	//the interface name should be given after the '%' symbol
	struct in6_addr tmp_addr;
	char *if_ptr = NULL;
	int ret, addr_type = 0;
	memset(&tmp_addr,0,sizeof(struct in6_addr));
#endif
	...
	if ((str = strstr(buf, "["))) {
		...
		*port_str = '\0'; /* Terminate string for IP */
		...
		ret = iscsi_pton6(str, (unsigned char *)&tmp_addr);
		addr_type = ipv6_addr_type(&tmp_addr);
		if(addr_type & IPV6_ADDR_LINKLOCAL) {
			if(if_ptr == NULL) {
				printk(KERN_ERR "Unable to locate \"%%ifname\""
					" in IPv6 iSCSI network portal link local address\n");
				return(ERR_PTR(-EINVAL));
			}
			else {
				//if this is link local address
				snprintf(np_addr.np_if_name, 16, "%s",if_ptr);
			}
		}
...
{									\
	...
	if(auth->name[strlen(auth->name)-1] == '\n')			\
		auth->name[strlen(auth->name)-1] = '\0'; /* Kill newline */ \
									\
...
static void lio_target_acl_release(struct config_item *acl_ci) 
{
...
static struct configfs_item_operations lio_target_acl_item_ops = {
	.release = lio_target_acl_release,
};

static struct config_item_type lio_target_acl_cit = {
	.ct_item_ops	= &lio_target_acl_item_ops,
		...
		const ssize_t max_pagelen)
{
	...
	se_session_t *se_sess = NULL, *se_sess_tmp = NULL;
	...
	if(is_empty == 0) {
		...
		list_for_each_entry_safe(se_sess, se_sess_tmp, &se_tpg->tpg_sess_list,
			...
			*/
			...
			spin_lock(&sess->conn_lock);
			for (conn = sess->conn_head; conn; conn = conn->next) {
				rb += snprintf(page+rb,(max_pagelen-rb), "CID: %hu  Connection State: ", conn->cid);
				switch (conn->conn_state) {
				case TARG_CONN_STATE_FREE:
					rb += snprintf(page+rb,(max_pagelen-rb), "TARG_CONN_STATE_FREE\n");
					break;
				case TARG_CONN_STATE_XPT_UP:
					rb += snprintf(page+rb,(max_pagelen-rb), "TARG_CONN_STATE_XPT_UP\n");
					break;
				case TARG_CONN_STATE_IN_LOGIN:
					rb += snprintf(page+rb,(max_pagelen-rb), "TARG_CONN_STATE_IN_LOGIN\n");
					break;
				case TARG_CONN_STATE_LOGGED_IN:
					rb += snprintf(page+rb,(max_pagelen-rb), "TARG_CONN_STATE_LOGGED_IN\n");
					break;
				case TARG_CONN_STATE_IN_LOGOUT:
					rb += snprintf(page+rb,(max_pagelen-rb), "TARG_CONN_STATE_IN_LOGOUT\n");
					break;
				case TARG_CONN_STATE_LOGOUT_REQUESTED:
					rb += snprintf(page+rb,(max_pagelen-rb), "TARG_CONN_STATE_LOGOUT_REQUESTED\n");
					break;
				case TARG_CONN_STATE_CLEANUP_WAIT:
					rb += snprintf(page+rb,(max_pagelen-rb), "TARG_CONN_STATE_CLEANUP_WAIT\n");
					break;
				default:
					rb += snprintf(page+rb,(max_pagelen-rb), "ERROR: Unknown Connection State!\n");
					break;
				}
				...
				rb += snprintf(page+rb,(max_pagelen-rb), "   Address %s %s \n", ip,
					(conn->network_transport == ISCSI_TCP) ? "TCP" : "SCTP");
				/* remove statSN
				rb += snprintf(page+rb,(max_pagelen-rb), "  StatSN: 0x%08x\n", conn->stat_sn);
				*/
			}
			spin_unlock(&sess->conn_lock);
		}
...
static ssize_t lio_target_show_tpg_session(void *p, char *page)
{
	iscsi_portal_group_t *tpg = (iscsi_portal_group_t *)p;
	ssize_t len = 0;
...
static struct lio_target_configfs_attribute lio_target_attr_tpg_session = {
	.attr = { .ca_owner = THIS_MODULE,
		    .ca_name = "sessions",
		    .ca_mode = S_IRUGO},
	.show = lio_target_show_tpg_session,
};

static struct configfs_attribute *lio_target_tpg_attrs[] = {
	&lio_target_attr_tpg_control.attr,
	&lio_target_attr_tpg_enable.attr,
	&lio_target_attr_tpg_session.attr,
	NULL,
};
        ...
        const char *name)
{
	...
	iscsi_node_attrib_t *nattr = NULL;
	iscsi_node_auth_t *auth = NULL;
	se_node_acl_t *se_nacl = NULL;
	struct config_group *nacl_cg = NULL;
	u32 cmdsn_depth = 0;
	const char def_pcy_name[] = "default_policy";
#endif
	...
	*/
	...
	cmdsn_depth = ISCSI_TPG_ATTRIB(tpg)->default_cmdsn_depth;
	acl = iscsi_tpg_add_initiator_node_acl(tpg, def_pcy_name, cmdsn_depth);
	if (!(acl))
		goto out;
	...
	config_group_init_type_name(&se_nacl->acl_group, def_pcy_name,
			&lio_target_initiator_cit);
	config_group_init_type_name(&se_nacl->acl_param_group, "param",
			&lio_target_nacl_param_cit);
	config_group_init_type_name(&nattr->acl_attrib_group, "attrib",
	...
	printk("LIO_Target_ConfigFS: REGISTER -> %s TPGT: %hu Initiator: %s"
		" CmdSN Depth: %u\n", config_item_name(tiqn_ci), tpg->tpgt,
		def_pcy_name, se_nacl->queue_depth);

	tpg->tpg_default_acl = acl;
	tpg->tpg_se_tpg->se_tpg_default_acl = (se_node_acl_t*)(&acl->se_node_acl);
	
	acl_cg = &tpg->tpg_acl_group;
...
#ifdef ALPHA_CUSTOMIZE //2011-10 tim tsay - show session info
//remove global session, because the PAGE_SIZE might not enough
static ssize_t lio_target_show_tiqn_sess_info(
	iscsi_tiqn_t *tiqn, 
	char *page,
	const ssize_t max_pagelen)
{
	ssize_t rb = 0;
	iscsi_portal_group_t *tpg = NULL, *tmp_tpg = NULL;

	if(tiqn == NULL || page == NULL)
		return 0;
	spin_lock(&tiqn->tiqn_state_lock);
	if (tiqn->tiqn_state == TIQN_STATE_ACTIVE) {
		spin_unlock(&tiqn->tiqn_state_lock);
		spin_lock(&tiqn->tiqn_tpg_lock);
		if(list_empty_careful(&tiqn->tiqn_tpg_list) == 0) {
			rb += sprintf(page, "\n---{ iSCSI Session Info "
		"for %s }---\n", tiqn->tiqn);
			list_for_each_entry_safe(tpg, tmp_tpg, 
				&tiqn->tiqn_tpg_list, tpg_list)
			{	
				iscsi_get_tpg(tpg);
				rb += lio_target_show_tgp_sess_info(
					tpg, page+rb, (max_pagelen-rb));
				iscsi_put_tpg(tpg);
				if(rb >= PAGE_SIZE) {
					rb += sprintf(page+PAGE_SIZE-17, "\n\tand so on ...\n");				
					break;
				}
			}
		}
...
static ssize_t lio_target_show_global_sess_info (char *page)
{
	ssize_t rb = 0;
	iscsi_tiqn_t *tiqn = NULL;
	
	
	if(page == NULL || iscsi_global->in_rmmod != 0 || 
		iscsi_global->in_shutdown != 0)
		return 0;
	spin_lock(&iscsi_global->tiqn_lock);
	list_for_each_entry(tiqn, &iscsi_global->g_tiqn_list, tiqn_list) {
		rb += lio_target_show_tiqn_sess_info(
			tiqn,page+rb,(PAGE_SIZE-rb));
		if(rb >= PAGE_SIZE) {
			break;
		}
	}
	...
	/*
	printk("page len = %llu/%lu\n",(unsigned long long)rb,(unsigned long)PAGE_SIZE);
	 */
	return rb;
}
static ssize_t lio_target_attr_show (struct config_item *item,
                                      ...
                                      char *page)
{
		if(strcmp(attr->ca_name,"sessions") == 0) {
			return(lio_target_show_global_sess_info(page));
		}
		else if(strcmp(attr->ca_name,"lio_version") == 0) {
...
#ifdef ALPHA_CUSTOMIZE //2011-10 Tim Tsay - support session info
static struct configfs_attribute lio_target_item_attr_sessions = {
	.ca_owner	= THIS_MODULE,
	.ca_name	= "sessions",
	.ca_mode	= S_IRUGO,
};
...
#ifdef ALPHA_CUSTOMIZE //2011-10 Tim Tsay - show iscsi global sessions info
static struct configfs_attribute *lio_target_attrs[] = {
	&lio_target_item_attr_version,
	&lio_target_item_attr_sessions,
	NULL,
};
