...
#ifdef ALPHA_CUSTOMIZE //2011-01-09 Tim Tsay - support target session info
...
static int get_net_ifindex_by_name(struct socket *sock, unsigned char *netif_name)
{
	...
	dev = dev_get_by_name(net, netif_name);
	if(!dev) {
		return -1;
	}
...
#endif
#ifdef ALPHA_CUSTOMIZE		//2012-01-03 Tim Tsay - support IPv6 link local address
		addr_type = ipv6_addr_type(&sock_in6.sin6_addr);
		if(strlen(np->np_if_name) > 0 && (addr_type & IPV6_ADDR_LINKLOCAL)) {
			sock_in6.sin6_scope_id = get_net_ifindex_by_name(sock, np->np_if_name);
			if(sock_in6.sin6_scope_id < 0) {
				sock_in6.sin6_scope_id = 0;
			}
		}
#endif
	} else {
...
#else
#ifdef ALPHA_CUSTOMIZE //2012-01-09 Tim Tsay - support target session infomations
		snprintf(conn->ipv6_login_ip, sizeof(conn->ipv6_login_ip), "%pI6c", &sock_in6.sin6_addr.in6_u);
#else
...
//#warning FIXME: Make iscsi_collect_login_stats() return correct failure
	...
	if(tpg && se_nacl) {
		spin_lock_bh(&tpg->tpg_se_tpg->acl_node_lock);
		if (se_nacl->nodeacl_flags & NAF_DYNAMIC_NODE_ACL) {
			if (!(ISCSI_TPG_ATTRIB(tpg)->cache_dynamic_acls)) {
				list_del(&se_nacl->acl_list);
				tpg->tpg_se_tpg->num_node_acls--;
				spin_unlock_bh(&tpg->tpg_se_tpg->acl_node_lock);
				core_free_device_list_for_node(se_nacl, tpg->tpg_se_tpg);
				lio_tpg_release_fabric_acl(tpg->tpg_se_tpg, se_nacl);
				spin_lock_bh(&tpg->tpg_se_tpg->acl_node_lock);
			}
		}
		spin_unlock_bh(&tpg->tpg_se_tpg->acl_node_lock);
