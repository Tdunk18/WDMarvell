	...
	int *ret)
{
	...
	if (np_addr->np_flags & NPF_NET_IPV6) {
		...
		if(strlen(np_addr->np_if_name) > 0) {
			memcpy(np->np_if_name, np_addr->np_if_name, 16);
		}
#endif
	} else {
...
#ifdef ALPHA_CUSTOMIZE /*2012-03-07 - Tim Tsay - set residual count
 	 	 	 	 	 	 Note: this has been fixed in linux 3.2 */
	if(cmd->se_cmd) {
		if(SE_CMD(cmd)->se_cmd_flags & SCF_UNDERFLOW_BIT) {
			if(SE_CMD(cmd)->residual_count > 0) {
				cmd->residual_count = SE_CMD(cmd)->residual_count;
			} else {
				cmd->residual_count = cmd->data_length - SE_CMD(cmd)->data_length;
			}
			cmd->data_length = SE_CMD(cmd)->data_length;
		}
		else if(SE_CMD(cmd)->se_cmd_flags & SCF_OVERFLOW_BIT) {
			if(SE_CMD(cmd)->residual_count > 0) {
				cmd->residual_count = SE_CMD(cmd)->residual_count;
			}
		}
...
void iscsi_thread_get_cpumask(struct iscsi_conn_s *conn, int thread_id)
{
	int ord, cpu;
	/*
	 ...
	 * execute upon.
	 */
	ord = thread_id % cpumask_weight(cpu_online_mask);
#if 1
