...
extern void lio_copy_node_attributes (
	se_node_acl_t *dst_se_acl,
	se_node_acl_t *src_se_acl)
{
	iscsi_node_acl_t *dst_acl = container_of(dst_se_acl, struct iscsi_node_acl_s,
					se_node_acl);
	iscsi_node_acl_t *src_acl = container_of(src_se_acl, struct iscsi_node_acl_s,
					se_node_acl);
	...
	int network_transport)
{
	...
	else if(strlen(np->np_if_name) > 0) {
		if(strcmp(np->np_if_name, np_addr->np_if_name)) {
			if (!(np = core_add_np(np_addr, network_transport, &ret)))
				return(ERR_PTR(ret));
		}
	}
