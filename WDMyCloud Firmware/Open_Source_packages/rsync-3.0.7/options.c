...
int cvs_exclude = 0;
int dry_run = 0;
int dump_size=0;  //add by alpha ibuki 2013/09/05
int have_daemon_opt=0;
int do_xfers = 1;
int ignore_times = 0;
int delete_mode = 0;
int delete_during = 0;
...
//royc 20100603
int do_count_progress = 0;			//flag to determine that do progress count or not
char tmp_file_name[256] = {0};		//combine to r_jobname
FILE *tmp_fp = NULL;				//file fp to write under /tmp/r_jobname
int table_flag = 0;
int count = 0;
int percent = 0;
char dump_size_file[256]={0};  //add by alpha ibuki 2013/09/05
//royc end
...
static struct poptOption long_options[] = {
  ...
  {"dump-size",         'n', POPT_ARG_NONE,   &dump_size, 0, 0, 0 },//add by alpha ibuki 2013/09/05
...
//royc 20100603
  {"job-name",         0,  POPT_ARG_STRING,   0, OPT_JOB_NAME, 0, 0 },
//royc end
  {0,0,0,0, 0, 0, 0}
};
...
//royc 20100603
		case OPT_JOB_NAME:
			arg = poptGetOptArg(pc);
			sprintf(tmp_file_name, "/tmp/r_%s", arg);
			sprintf(dump_size_file,"%s.dump_size",tmp_file_name);  //add by alpha ibuki 2013/09/05
...
//add by alpha ibuki 2013/09/05
	if(dry_run&&dump_size&&strlen(dump_size_file)>0)
