...
void do_dump_size_file()
{
	char cmd[512]={0};
	if(strlen(dump_size_file)>0){
		sprintf(cmd,"echo %llu > %s",stats.total_transferred_size,dump_size_file);
		system(cmd);
	}
}
...
//royc 20100603	
	//when rsync job is done, write 100 to tmp file
	//modify by alpha ibuki 2013/09/05
	if(dry_run&&dump_size)
		do_dump_size_file();
	else
