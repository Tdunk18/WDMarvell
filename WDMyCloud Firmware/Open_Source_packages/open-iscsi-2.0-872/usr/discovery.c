...
#ifdef ALPHA_CUSTOMIZE //2012-02-15 Tim Tsay - support to discovery IPv6 link-local address
	if(iface && strlen(iface->netdev))
	{
		session->conn[0].ipv6_scope_id = if_nametoindex(iface->netdev);
	}
	else
	{
		session->conn[0].ipv6_scope_id = 0;
	}
#endif
