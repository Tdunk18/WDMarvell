...
__conn_error_handle(iscsi_session_t *session, iscsi_conn_t *conn)
{
	...
	case STATE_LOGGED_IN:
		...
		// 20110811 - tim tsay - notify conn error event to virtual
		// volume controller
		{
			char cmd[300];
			memset(cmd, 0, sizeof(cmd));
			sprintf(cmd, "vvctl --oiscsi_evt_reopen -n \"%s\" &", session->target_name);
			system(cmd);
		}
#endif
		break;
...
#ifdef ALPHA_CUSTOMIZE
	// 20110811 - tim tsay - notify conn recovered event to virtual
	// volume controller
	int flag_recovering_conn = 0;
	char syscmd[300];
...
#ifdef ALPHA_CUSTOMIZE
	// 20110811 - tim tsay - notify conn recovered from reopen state
	// to virtual volume controller
	if(flag_recovering_conn != 0){
		memset(syscmd, 0, sizeof(syscmd));
		sprintf(syscmd, "vvctl --oiscsi_evt_recover -n \"%s\" &",
			session->target_name);
		system(syscmd);
	}
#endif
