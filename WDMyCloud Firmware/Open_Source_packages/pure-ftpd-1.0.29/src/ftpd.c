...
#define DONT_LOG_IP	1	//alpha.jack.20090615+
#define WITHOUT_ASCII	1

int allowed = 0;
//char IE_send_utf8 = 0; //jack20080110+
char client_language = NON_UTF8;
unsigned char auth_name[2048];	// patrick 20110420
unsigned char auth_path[2048];	// patrick 20110420
static int pfd1[2], pfd2[2];
char tls_login='0';

void printf_out(char *fmt, ...)
{
	va_list ap;
	int fd;
...
// add by Patrick.alpha 20090812, 2011/11/11 revise for IPv6 ++
static int check_burst_tries(const struct sockaddr_storage * const addr) {

    char buf[64], addrbuf[64]= {0}, blockedaddr[64]= {0};
	unsigned long a1=0U, a2=0U;
	unsigned int b1, b2, b3, b4;
    int retval=0, block_flag=0;
	FILE *fptr;

    if(STORAGE_FAMILY(*addr)==AF_INET) {
	a1=ntohl(STORAGE_SIN_ADDR(*addr));
        inet_ntop(addr->ss_family,
                  &((struct sockaddr_in *)addr)->sin_addr, addrbuf, 64);
        //logfile(LOG_INFO, "ipv4 address: [%lu]: %s\n", a1, addrbuf);
    }
    else {
        inet_ntop(addr->ss_family,
                  &((struct sockaddr_in6 *)addr)->sin6_addr, addrbuf, 64);
    }
	fptr=fopen("/etc/blockip", "r");
	if(fptr==NULL) {
		fptr=fopen("/etc/blockip", "w");
		fclose(fptr);
		return -1;
	}
	else {
		while(fgets(buf, sizeof(buf), fptr)!=NULL) {
            if(((retval=sscanf(buf, "%[^,]%*[^\r\n\b]%*c", blockedaddr))==1) && retval != EOF) {
                //logfile(LOG_INFO, "2: ipv4 address: [%lu]: %s\n", a1, blockedaddr);
                if(strcasecmp(addrbuf, blockedaddr)==0) {
                    block_flag=-1;
                    //logfile(LOG_INFO, "%d\n", block_flag);
					break;
			}
...
/*
static int check_burst_tries_r(const struct sockaddr_storage *const addr) {
	char buf[64]="", *cptr=NULL;
	unsigned long a1=0U, a2=0U, a3=0U;
	unsigned int b1, b2, b3, b4, e1, e2, e3, e4;
	FILE *fptr;
	a1=ntohl(STORAGE_SIN_ADDR(*addr));
	if((fptr=fopen("/etc/blockip", "r"))==NULL) {
		fptr=fopen("/etc/blockip", "w");
		fclose(fptr);
		return -1;
	}
		...
		while(fgets(buf, sizeof(buf), fptr)!=NULL) {		
			if((cptr=strchr(buf, '~'))!=NULL) {	// ip range format is x.x.x.x~x.x.x.x
				a2=a3=b1=b2=b3=b4=e1=e2=e3=e4=0;
        if ((sscanf(buf, "%u.%u.%u.%u~%u.%u.%u.%u%*c", &b1, &b2, &b3, &b4, &e1, &e2, &e3, &e4) != 8) ||
        	b1 > 255U || b2 > 255U || b3 > 255U || b4 > 255U ||
        	(b1 | b2 | b3 | b4) == 0U ||
        	e1 > 255U || e2 > 255U || e3 > 255U || e4 > 255U ||
        	(e1 | e2 | e3 | e4) == 0U)
            continue;
				a2 = b1 << 24 | b2 << 16 | b3 << 8 | b4;
        a3 = e1 << 24 | e2 << 16 | e3 << 8 | e4;
        if(a2<a1&&a1<a3) {
        	fclose(fptr);
        	return -1;
        }
			}
			else { //individual block ip
				a2=b1=b2=b3=b4=0;
				if(sscanf(buf, "%u.%u.%u.%u%*c", &b1, &b2, &b3, &b4)==4) {
					a2 = b1 << 24 | b2 << 16 | b3 << 8 | b4;
					if(a1==a2) {
						fclose(fptr);
						return -1;
...
static int blockipfunction(const char *configfile) {
	
	FILE *fptr;
	char buf[128], *cptr=NULL;
	int blockflag=-1;
	...
	while(fgets(buf, 128, fptr)!=NULL) {
		if((cptr=strstr(buf, "<blockipstatus>1</blockipstatus>"))!=NULL) 
			blockflag=1;
		else if((cptr=strstr(buf, "<blockipstatus>0</blockipstatus>"))!=NULL)
			blockflag=0;
	}
	...
	//return blockflag;
	return 1; //default enable it in current f/w version (2010/12/13)
}

static void get_remote_ip(const struct sockaddr_storage * const addr, char* Dest) {
	unsigned long a1=0U;
    char buf[64]= {0};
    if(STORAGE_FAMILY(*addr)==AF_INET) {
	a1=ntohl(STORAGE_SIN_ADDR(*addr));
        inet_ntop(addr->ss_family,
                  &((struct sockaddr_in *)addr)->sin_addr, buf, 64);

        //logfile(LOG_INFO, "ipv4 address: [%lu]: %s\n", a1, buf);
        strncpy(Dest, buf, strlen(buf));
//		if(a1!=0) {
//		    sprintf(Dest, "%d.%d.%d.%d", (a1>>24)&0xff, (a1>>16)&0xff, (a1>>8)&0xff, a1&0xff);
//		}
    }
    else if(STORAGE_FAMILY(*addr)==AF_INET6) {
        //a1=ntohl(STORAGE_SIN_ADDR6(*addr));
        inet_ntop(addr->ss_family,
                  &((struct sockaddr_in6 *)addr)->sin6_addr, buf, 64);
        //logfile(LOG_INFO, "ipv6 address: [%lu]: %s\n", a1, buf);
        strncpy(Dest, buf, strlen(buf));
    }

}

static int get_xml_value(const char *tag) {
	int value=0, ret=0;
	FILE *pfptr=NULL, *fptr=NULL;
	char *cptr=NULL, buffer[128]={0};
	
	// Use fopen instead of popen
	
	fptr=fopen("/etc/NAS_CFG/config.xml", "r");
	if(!fptr) return -1;
	if(!feof(fptr) && !ferror(fptr)) {
		while((ret=fscanf(fptr, "%[^\n\b\r]%*c", buffer))!=0 && ret!=EOF) {
			if((cptr=strstr(buffer, tag))==NULL) {
				value=5;	// default set to 5
			}
			else {
				cptr=cptr+strlen(tag)+1;
				value=atoi(cptr);
				break;
			}
		}
	...
	snprintf(command, sizeof(command), "echo `grep '%s' /etc/NAS_CFG/config.xml` ", tag);
	pfptr=popen(command, "r");
	if(pfptr!=NULL) {
		if(fscanf(pfptr, "%*[^>]%*c%d%*s", &value)!=1) {
			value=1;
			
			if (strstr(tag, "retryfails")) {
				value=5;		// retry fails default is 5 times
			}
			else if (strstr(tag, "monitorduration")) {
				value=5;		// retry fails default is 5 times
			}
			else if (strstr(tag, "blockduration")) {
				value=5;		// retry fails default is 5 times
			}
		}
	pclose(pfptr);
...
# endif
    /*syslog(crit, "(%s@%s) %s%s",
           ...
           urgency, line); */
	openlog("FTP",0,0);
    syslog(crit, "FTP server: %s", line);    // Wilson ask add 'FTP server:' this message in log.
	closelog();
# ifdef SAVE_DESCRIPTORS
    closelog();
# endif
...
//                    nextentry == NULL ? ' ' : '-', scannedentry->line);// Patrick mark here
        }       
...
#endif
    while (*namepnt != 0) {
#if 0 //alpha.jack.20070413+ for specically char   	
...
/*alpah.jack.20100702- for hidden file can access */ 
#if 0
            if (namepnt[0] == '.') {   /* /. */
                if (namepnt[1] == 0) { /* /.$ => ok */
                    return 0;
                 ...
                 const char * const prefix)
{
    ...
    /*alpha.jack.20101224 for hidden some folder */
    if (S_ISDIR(st.st_mode))
    {	
	    if ( check_hidden_folder(file) == 1 )
	    {
	    	return -1;
	    }
	 }
	 
	 if ( Check_Share_List(file) == -1 )
	 	return -1;

    /*alpha.jack.20101224 end*/
...
/*alpha.jack.20101013+ don't send client symlink */    	
//        ssize_t sx;
//        ft = "OS.unix=symlink";        
...
//alpha.jack.20100902- for FileZilla or FTPrush use anonynmous connnect ftp server , share folder incorrect
//    if (guest != 0) {
...
void douser(const char *username)
{
    struct passwd *pw = NULL;
     char destaddr[32];
//alpha.jack.20100629+ for pop a window to user login    
    char share_root[64];
    char ftp_home[64];
    DIR *dir;
    struct dirent *ptr;
    ...
    memset(ftp_home,0,64);
//Alpha.Andrew_modify_20130619 for WD to support admin user
#if defined(PROJECT_FEATURE_CUSTOM_DLINK)
    if (!strcasecmp(username, "admin") || !strcasecmp(username, "nobody") || !strcasecmp(username, "root"))
		return;
#elif defined(PROJECT_FEATURE_CUSTOM_WD)
	if (!strcasecmp(username, "nobody") || !strcasecmp(username, "root"))
		return;
#endif
	strncpy(auth_name, username, strlen(username));
	strcpy(ftp_home,"/usr/share/ftp");
	strcpy(auth_path,"/usr/share/ftp");
	
#if defined(PROJECT_FEATURE_CUSTOM_WD)
	if(!strcasecmp(username, "admin"))
	{
		...
		fp = fopen("/etc/passwd_tmp", "w+");
		while((user = getpwent()) != 0 )
		{
			if(user->pw_uid == 500)
			{
				fprintf(fp, "%s:x:%d:%d:%s:%s:%s\n", user->pw_name, user->pw_uid, user->pw_gid, user->pw_gecos, "/usr/share/ftp", user->pw_shell);
			}
			else
			{
		...
		remove("/etc/passwd");
		rename("/etc/passwd_tmp", "/etc/passwd");
	}
#endif

	if((Get_Share(username) == 0) && (strcmp(username, "ftp") == 0 || strcmp(username, "anonymous") == 0))
	{
		addreply_noformat(530, MSG_NO_ANONYMOUS_LOGIN);
		return;
	}
    ...
    if (anon_only <= 0 && username != NULL && *username != 0 &&
...
#endif
        {
            ...
            memset(destaddr, 0x00, sizeof(destaddr));	// ++ alpha.patrick 2009/11/20 
            get_remote_ip(&peer, destaddr);
            logfile(LOG_INFO, "Anonymous user logged in from %s", destaddr);		// alpha.patrick 2009/11/20 ++
        }
#ifdef WITH_VIRTUAL_HOSTS
        else {                       /* virtual host */
...
void dopass(char *password)
{
    ...
    char *hd;
    int ngroups;
    char countbuf[8], destaddr[32];
    unsigned int retries=get_xml_value("retryfails");
    ...
    if (authresult.auth_ok != 1) {
        ...
        if(blockipfunction("/etc/NAS_CFG/config.xml")==1) {
	        // Read tapping from pipe
	        close(pfd2[1]);
	        memset(countbuf, 0x00, sizeof(countbuf));
	        read(pfd2[0], countbuf, sizeof(countbuf));
	        close(pfd2[0]);
	        sscanf(countbuf, "%d", &counts);
	        // Write tapping to pipe
					close(pfd1[0]);
	        memset(countbuf, 0x00, sizeof(countbuf));
	        sprintf(countbuf, "%d", tapping);
	        write(pfd1[1], countbuf, strlen(countbuf));
	        close(pfd1[1]);
      	}
        randomsleep(tapping);
        ...
        //if (tapping > MAX_PASSWD_TRIES) {
				if(retries<counts) {
//            logfile(LOG_ERR, MSG_AUTH_TOOMANY);// Patrick mark here
            _EXIT(EXIT_FAILURE);
        }
...
//        logfile(LOG_WARNING, MSG_ACCOUNT_DISABLED, account);// Patrick mark here
        randomsleep(tapping);
        if (retries<=counts) {
        //if (tapping >= MAX_PASSWD_TRIES) {
            addreply_noformat(530, MSG_AUTH_FAILED);
    ...
    // here add actual home directory in NAS    
    //printf_out("authresult.dir: %s:(%s)\n", authresult.dir, auth_path);	// patrick test
    free((void *) authresult.dir);
    authresult.dir = strdup(auth_path);
...
# endif
    ...
    get_remote_ip(&peer, destaddr);
    logfile(LOG_INFO, "%s user logged in from %s %s", account, destaddr, (tls_login=='1') ? ("over SSL/TLS") : (""));
#ifdef FTPWHO
    if (shm_data_cur != NULL) {
        ftpwho_lock();
        strncpy(shm_data_cur->account, account,
...
/* alpha.jack.20091013+ */
	...
	char check_allow[MAXPATHLEN];
	int i,count=0;
	char *p;
/* alpha.jack.20091013 end */
	char wd_tmp[MAXPATHLEN] = {0};	//Andrew_add_20130628
	...
	//Alpha_Andrew_add_start_20130628
	if(*where == '/')
	{
		strcpy(wd_tmp, where);
	}
	...
	//logfile(LOG_INFO, "-->wd_tmp[%s]", wd_tmp);
	allowed = check_allowed(wd_tmp, allowed);
	if(allowed == 0)
	{
		addreply_noformat(553, "User not allow");
		return;
	}
	//Alpha_Andrew_add_end_20130628
    ...
    if (chdir(where) != 0) {
        ...
        //logfile(LOG_INFO, "%s", buffer);  //alpha.jack.20100908-
...
# endif
...
#ifdef WITH_RFC2640
    nwd = charset_fs2client(wd);
    /*alpha.jack.20100602+ for judge user */
    //allowed = check_allowed(wd,allowed);
#if 0
	//Alpha_andrew_add_start_20120710
	allowed = check_allowed(wd,allowed);
	if ( allowed == 0 )
	{
		addreply_noformat(553, "User not allow");
	...
	//Alpha_andrew_add_end_20120710
#endif
//    if ( strlen(wd) > 1 )
//    {
//      strcpy(check_allow,wd);
//    	
//    	p = strchr( check_allow+1,'/');
//    	
//    	if ( p != NULL )
//    		*p=0;
//    	lstat(check_allow,&file_state);
//    	if ( file_state.st_uid == 0 )
//    		allowed = 1;
//    	else
//    		allowed = 0;
//    }
    /*alpha.jack.20100602 end */
...
/*alpha.jack.20100818+ */
	allowed = check_allowed(name,allowed);		//Andrew_add_20130603
	if ( allowed == 0 || allowed == 1)		//Andrew_modify_20121017
	{
...
/*alpha.jack.20100818+*/
	allowed = check_allowed(name,allowed);		//Andrew_add_20130603
	if ( allowed == 0 || allowed == 1)		//Andrew_modify_20121029
	{
...
//            ? "/" : "", name);
	...
	//logfile(LOG_NOTICE, "Deleted %s/%s", wd,name);
	char destaddr[32];
    memset(destaddr, 0x00, sizeof(destaddr));
    get_remote_ip(&peer, destaddr);
	logfile(LOG_NOTICE, "Deleted %s/%s by %s[%s]", wd, name, (!strcmp(account, "ftp")) ? "anonymous user" : account, destaddr);	//Alpha_Andrew add for WD 20140211
    return;
                        ...
                        const char * const name, int up)
{
    ...
    if (!SNCHECK(snprintf(speedstring, sizeof speedstring,
                 ...
                 sizeof speedstring)) {
//alpha.jack.2010.0927                 	
//        logfile(LOG_NOTICE, "%s%s%s%s %s %s", root_directory,
//                *name == '/' ? "" : wd,
//                (*name != '/' && (!*wd || wd[strlen(wd) - 1] != '/'))
//                ? "/" : "", name, word, speedstring);
					 
		//logfile(LOG_NOTICE, "%s/%s %s %s", wd,name,word,speedstring);
		char destaddr[32];
		memset(destaddr, 0x00, sizeof(destaddr));
		get_remote_ip(&peer, destaddr);
		logfile(LOG_NOTICE, "%s/%s %s %s by %s[%s]", wd, name, word, speedstring, (!strcmp(account, "ftp")) ? "anonymous user" : account, destaddr);	//Alpha_Andrew add for WD 20140211
    }
...
//                    name, strerror(errno));            // Patrick mark here
            return;
        }
...
void doretr(char *name)
{
	...
	//Alpha_andrew_add_start_20120710
	allowed = check_allowed(name,allowed);
	if ( allowed == 0 )
	{
		addreply_noformat(553, "User not allow");
		goto end;
...
void dorest(const char *name)
{
    ...
    if (*endptr != 0 || restartat < (off_t) 0) {
        ...
        if (type == 1 && restartat != 0) {
            ...
            if (restartat != 0) {
                //logfile(LOG_NOTICE, MSG_REST_SUCCESS, (long long) restartat); //alpha.jack.20100908-
            }
...
/*alpha.jack.20100818+*/  
	allowed = check_allowed(name,allowed);		//Andrew_add_20130603
	if (allowed == 0 || allowed == 1)	//Andrew_modify_20121001
	{
...
/*alpha.jack.20100818 end*/
    ...
    if ((mkdir(name, (mode_t) (0777 & ~u_mask_d))) < 0) {
        ...
        //logfile(LOG_NOTICE, "The directory %s was successfully created", name);    // Patrick add 2009/10/30
		char destaddr[32];
		memset(destaddr, 0x00, sizeof(destaddr));
		get_remote_ip(&peer, destaddr);
		logfile(LOG_NOTICE, "The directory %s was successfully created by %s[%s]", name, (!strcmp(account, "ftp")) ? "anonymous user" : account, destaddr);	//Alpha_Andrew add for WD 20140211
...
/*alpha.jack.20100818 end*/
    ...
    if ((rmdir(name)) < 0) {
        ...
        //logfile(LOG_NOTICE, "The directory %s was successfully removed", name);    // Patrick add 2009/10/30
		char destaddr[32];
		memset(destaddr, 0x00, sizeof(destaddr));
		get_remote_ip(&peer, destaddr);
		logfile(LOG_NOTICE, "The directory %s was successfully removed by %s[%s]", name, (!strcmp(account, "ftp")) ? "anonymous user" : account, destaddr);	//Alpha_Andrew add for WD 20140211
...
//    char feat[] = FEAT FEAT_DEBUG FEAT_TLS FEAT_UTF8 FEAT_TVFS FEAT_ESTA FEAT_PASV FEAT_ESTP;
    char feat[512];
    if ( client_language == UTF8 )
	{
		sprintf(feat,"%s%s%s%s%s%s%s%s",FEAT,FEAT_DEBUG,FEAT_TVFS,FEAT_ESTP,FEAT_PASV,FEAT_ESTA,FEAT_TLS,FEAT_UTF8);
	}
	else
		sprintf(feat,"%s%s%s%s%s%s%s",FEAT,FEAT_DEBUG,FEAT_TVFS,FEAT_ESTP,FEAT_PASV,FEAT_ESTA,FEAT_TLS);
/*alpha.jack.20100818 end*/ 		
...
/*alpha.jack.20070830+*/
	allowed = check_allowed(name,allowed);
	if (allowed == 0 || allowed == 1)	//Andrew_modify_20120810
	{
...
/*alpha.jack.20100818 end*/
 ...
 //               renamefrom, name); // Patrick mark here
    }
    bye:
    (void) free(renamefrom);
    renamefrom = NULL;
...
# ifdef WITH_RFC2640
	if ( client_language == UTF8 ) /*alpha.jack.20100818+*/
	{
...
//    logfile(LOG_ERR, "%s: %s", msg, e);// Patrick mark here
...
//        logfile(LOG_ERR, MSG_OPEN_FAILURE, fortunes_file); // Patrick mark here
        return -1;
    }
    if (fstat(fd, &st) < 0 ||
...
#endif
    iptropize(&peer);
    //logfile(LOG_INFO, MSG_NEW_CONNECTION, host); //alpha.Patrick mark here
...
# endif
    ...
    if (altlog_format != ALTLOG_NONE) {
        ...
        if (altlog_fd == -1) {
//            logfile(LOG_ERR, "altlog %s: %s", altlog_filename, strerror(errno));// Patrick mark here
        }
    }
#endif
...
# endif
    ...
    //logfile(LOG_INFO, MSG_LOGOUT); // Patrick mark here
	char destaddr[32];
	memset(destaddr, 0x00, sizeof(destaddr));
	get_remote_ip(&peer, destaddr);
    logfile(LOG_INFO, "%s[%s] Logout", (!strcmp(account, "ftp")) ? "Anonymous user" : account, destaddr);
    doreply();
#ifdef WITH_BONJOUR
    refreshManager();
#endif
...
//            logfile(LOG_ERR, MSG_STANDALONE_FAILED ": [fork: %s]", strerror(errno));// Patrick mark here
            return;
...
static void accept_client(const int active_listen_fd) {
	fprintf(stderr, "accept client routine\n");
	//Patrick 20090817 ++
	char countbuf[8]="";
	int status, readbytes=0, counts=0;
	unsigned long a1=0U, a2=0U;
	unsigned int b1,b2,b3,b4;
	FILE *fptr=NULL;
	fd_set rs, ws;
    ...
    pid_t child;
unsigned int retries=get_xml_value("retryfails");
    char cli_addr[64]={0};	// patrick 2011.11.14 ++
	//get_volume_map(); /*alpha.jack.20101224+ for hidden some folder */
    ...
    if (maxusers > 0U && nb_children >= maxusers) {
        ...
        (void) close(clientfd);
        clientfd = -1;
		closeallpipe(); // close all pipe fds patrick ++
        return;
    }
    if (maxip > 0U) {
        fourinsix(&sa);
...
//                            (unsigned long) maxip, hbuf);// Patrick mark here
                }
            }
            (void) close(clientfd);
            clientfd = -1;
	...
	//Patrick 20090814 ++
    if(1==blockipfunction("/etc/NAS_CFG/config.xml")) {
  	  if(check_burst_tries(&sa)==-1) {
				(void) close(clientfd);
				clientfd = -1;
				closeallpipe();// close all pipe fds
            return;
        }
    }
    // Patrick 20090814 --
	...
	else {// Patrick 20090818 ++
    	if(1==blockipfunction("/etc/NAS_CFG/config.xml")) {
				//write current count to children
				close(pfd2[0]);
				memset(countbuf, 0x00, sizeof(countbuf));
				sprintf(countbuf, "%d", tryings);
				write(pfd2[1], countbuf, strlen(countbuf));
				close(pfd2[1]);
				// Read counting from children process
				rc=select(pfd1[1]+1, NULL, &ws, NULL, &tv);
				if(rc==0)
					close(pfd1[1]);
				else {
					if(FD_ISSET(pfd1[1], &ws)) {
						close(pfd1[1]);		// Close pipe write fd
					}
				}
				memset(countbuf, 0x00, sizeof(countbuf));
				//waitpid(child, &status, WNOHANG);

				rc=select(pfd1[0]+1, &rs, NULL, NULL, &tv);
				if(rc==0)
					close(pfd1[0]);
				else {
					if(FD_ISSET(pfd1[0], &rs)) {
						readbytes=read(pfd1[0], countbuf, sizeof(countbuf));
						close(pfd1[0]);
					}
				}
				
				sscanf(countbuf, "%d", &counts);
            //a1=ntohl(STORAGE_SIN_ADDR(sa));
            if(STORAGE_FAMILY(sa)==AF_INET) {
                inet_ntop(sa.ss_family, &((struct sockaddr_in *)&sa)->sin_addr, cli_addr, 64);
//                logfile(LOG_INFO, "IPv4: %s\n", cli_addr);
            }
            else {
                inet_ntop(sa.ss_family, &((struct sockaddr_in6 *)&sa)->sin6_addr, cli_addr, 64);
//                logfile(LOG_INFO, "IPv6: %s\n", cli_addr);
            }
            /* Use char type to store accept address */
            //memset(previousip, 0, sizeof(previousip));
				if(counts==1) {	// receive 'tapping' value in dopass
                //if(a1==previousip) {
                if(strcmp(cli_addr, previousip)==0) {
						time(&ts);
						if(ts<=timestamp) {
						tryings++;
					}
							...
							tryings=1;
							time(&timestamp);
							timestamp+=get_xml_value("monitorduration")*60UL;
						}
					}
					else {	// the 1st typing error, record this timestamp
                    //previousip=a1;
                    strncpy(previousip, cli_addr, strlen(cli_addr));
//                    logfile(LOG_INFO, "%s\n", cli_addr);
						tryings=1;
						time(&timestamp);
						timestamp+=get_xml_value("monitorduration")*60UL;
					...
					tryings=0;
				if(retries<=tryings) {// Auto-ban, Write incoming IP to block list
	    	//if(tryings>=MAX_PASSWD_TRIES) {
	    		memset(now_time, 0x00, sizeof(now_time));
	    		time(&ts);
	    		ts+=get_xml_value("blockduration")*60UL;
					tp=localtime(&ts);
					snprintf(now_time, sizeof(now_time), "%4d%02d%02d%02d%02d", 1900+tp->tm_year, 1+tp->tm_mon, tp->tm_mday, tp->tm_hour, tp->tm_min);
	    		fptr=fopen("/etc/blockip", "a+");
                //fprintf(fptr, "%s,0,%s,1\n", cli_addr, now_time);
				fprintf(fptr, "%s,1,------------,1\n", cli_addr);	//Andrew_modify_20130117
//                logfile(LOG_INFO, "should be blocked IP address: %s\n", cli_addr);
                //fprintf(fptr, "%lu.%lu.%lu.%lu,0,%s,1\n", (a1 >> 24) & 255, (a1 >> 16) & 255, (a1 >> 8) & 255, a1 & 255, now_time);
				fflush(fptr);
	    		fclose(fptr);
	    		system("touch /tmp/update_blockip");
	    		tryings=0;
	    	}
    	}
			else {
    		closeallpipe();// close all unused pipe fd 2010/10/18
    	}	// Patrick 20090818 --
        if (maxip > 0U) {
...
//            logfile(LOG_ERR, MSG_STANDALONE_FAILED ": [%s]", strerror(old_errno)); // Patrick mark here
            return;
        }
...
# endif
    ...
    fprintf(stderr, "End of Standalone server\n");
}
#endif
    ...
    while ((fodder =
        ...
        case 's': {
            if ((pw = getpwnam("ftp"))) {
                ...
                //logfile(LOG_ERR, MSG_NO_FTP_ACCOUNT); //alpha.jack.20100908-
            }
            break;
        }
        case '0': {
...
/*alpha.jack.20100818+*/            
	    if(!strcasecmp(charset_client, "utf-8"))	
	    	client_language=UTF8;
/*alpha.jack.20100818 end*/  	    	
            break;
        }
#endif 
...
//                logfile(LOG_ERR, 
//                        MSG_CONF_ERR ": " MSG_ILLEGAL_FACILITY ": %s", optarg);// Patrick mark here
            }
            break;
        }
        case 'l': {
...
//        default:
//            logfile(LOG_WARNING, MSG_ILLEGAL_OPTION);// Patrick mark here
        }
    }
#ifdef WITH_RFC2640
    if (charset_fs == NULL) {
   ...
   if(client_language==UTF8) /*alpha.jack.20100818+*/
   {
    ...
    if (firstport) {
        ...
        if (!maxusers || maxusers > portmax) {
            //maxusers = portmax;    /* ... so we don't run out of ports */  // 20150202 Jacky mask, when passive mode and only 2 port used, FileZilla can not transfer files.
        }
    }
    if (bypass_ipv6 == 0) {
        check_ipv6_support();
...
#ifdef WITH_DIRALIASES
    if (init_aliases() != 0) {
//        logfile(LOG_ERR, MSG_ALIASES_BROKEN_FILE);// Patrick mark here
    }
#endif
#ifdef WITH_TLS
