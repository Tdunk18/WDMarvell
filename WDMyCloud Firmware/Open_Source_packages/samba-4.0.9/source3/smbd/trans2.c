...
//20121018 roy
	...
	if (rval && (ra_type != RA_SAMBA) && (ra_type != RA_CIFSFS)&& (ra_type != RA_OSX)) {
#else
...
#ifdef ALPHA_CUSTOMIZE
			if(strcmp(conn->connectpath, "/etc/dfsroot") == 0)
			{
				dsize = 1048576*128;
				dfree = 1048576*128;
			}
...
//20121018 roy
#ifdef ALPHA_CUSTOMIZE
			if (RA_OSX == get_remote_arch()) {
				/* MAC 10.6 can not support CIFS_UNIX_POSIX_PATH_OPERATIONS_CAP */
