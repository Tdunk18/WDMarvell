...
#ifdef ALPHA_CUSTOMIZE
			openlog("SAMBA", 0, 0);
			syslog(LOG_WARNING, CIFS_AUTH_FAIL, "nobody");
			closelog();
#endif
//royc end ==========================================================
...
#ifdef ALPHA_CUSTOMIZE
			openlog("SAMBA", 0, 0);
			syslog(LOG_WARNING, CIFS_AUTH_FAIL, session_info->unix_info->unix_name);
			closelog();
#endif
//royc end ==========================================================
...
//royc 20100503 [IPC$] [lp] ==========================================================
#ifdef ALPHA_CUSTOMIZE
	if(conn->session_info->unix_info->unix_name != NULL && conn != NULL &&
		!(strncmp(lp_servicename(talloc_tos(), snum), "IPC$", strlen("IPC$")) == 0
		 || strncmp(lp_servicename(talloc_tos(), snum), "lp", strlen("lp")) == 0 ) )
	{
		DEBUG(0, (CIFS_CONNECT, 
		  tsocket_address_string(conn->sconn->remote_address,
						talloc_tos()), lp_servicename(talloc_tos(), snum), conn->session_info->unix_info->unix_name));
		  
		openlog("SAMBA", 0, 0);
		syslog(LOG_INFO, CIFS_CONNECT, 
		  tsocket_address_string(conn->sconn->remote_address,
						talloc_tos()), lp_servicename(talloc_tos(), snum), conn->session_info->unix_info->unix_name);
		closelog();
	}
...
#ifdef ALPHA_CUSTOMIZE	
	if(conn != NULL && 
		!(strncmp(lp_servicename(talloc_tos(), SNUM(conn)), "IPC$", strlen("IPC$")) == 0
		 || strncmp(lp_servicename(talloc_tos(), SNUM(conn)), "lp", strlen("lp")) == 0 ) )
	{
		DEBUG(0, (CIFS_DISCONN, 
		  tsocket_address_string(conn->sconn->remote_address,
							talloc_tos()), lp_servicename(talloc_tos(), SNUM(conn))));
		  
		openlog("SAMBA", 0, 0);
		syslog(LOG_INFO, CIFS_DISCONN, tsocket_address_string(conn->sconn->remote_address,
							talloc_tos()), lp_servicename(talloc_tos(), SNUM(conn)));
		closelog();
	}
#endif
