...
static int vfswrap_ftruncate(vfs_handle_struct *handle, files_struct *fsp, off_t len)
{
...
#ifdef ALPHA_CUSTOMIZE //Matt, 2012/4/23
	/* To fix VFAT sys_ftruncate spent long time issue.
	  * EXT4/ntfs-3g are okay.
	 */
	...
	{
		DEBUG(10, ("vfswrap_ftruncate(), is FIFO mode\n"));
		result=0;
		goto done;
	}
#endif
	
	if (st.st_ex_size <= len) /* the file size is less than requested size already */
	{
		result = 0;
		goto done;
	}
#endif  //End ALPHA_CUSTOMIZE
