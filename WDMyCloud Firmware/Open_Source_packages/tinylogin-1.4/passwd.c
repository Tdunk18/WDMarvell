...
/*Get "echo" password for passwd +20141808.VODKA*/
static char *getpass_alpha(char *msg)
{
...
static int new_password(const struct passwd *pw, int amroot, int algo)
{
	...
	if (!amroot && crypt_passwd[0]) {
		if (!(clear = getpass_alpha("Old password:"))) {
			/* return -1; */
			return 1;
		}
	...
	if (!
		(cp =
		 getpass_alpha ("Enter the new password (minimum of 5, maximum of 8 characters)\n"
	...
	if (!(cp = getpass_alpha("Re-enter new password: "))) {
		bzero(orig, sizeof orig);
		/* return -1; */
		return 1;
	}
