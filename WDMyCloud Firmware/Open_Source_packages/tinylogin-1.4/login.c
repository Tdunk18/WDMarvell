...
#ifdef ALPHA_CUSTOMIZE
void get_admin_name(char *admin_name)
{
	FILE  	*fp;
	char	line[USERNAME_SIZE] = {0};
	int		i, len;
	
	if((fp = popen("awk -F: '($3==500) {print $1}' /etc/passwd", "r")) == NULL)
	{
		//default value
		safe_strncpy(admin_name, "admin", 5);
	}
	else
	{
		fgets(line, USERNAME_SIZE, fp);
		pclose(fp);
		
		len = strlen(line);
		for( i=len-1; i>=0; i--)
		{
			if( iscntrl(*(line+i)) || isspace(*(line+i)))
				*(line+i) = 0;
			else
				break;
		}
		
		safe_strncpy(admin_name, line, USERNAME_SIZE);
	}
}
#endif
...
#ifdef ALPHA_CUSTOMIZE
		/* Force to use "admin" */
		//snprintf(username, USERNAME_SIZE-1, "admin");
		get_admin_name(username);
#else
		if ( !username[0] )
