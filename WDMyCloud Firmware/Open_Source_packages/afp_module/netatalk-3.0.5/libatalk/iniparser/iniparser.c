...
#define ASCIILINESZ         (6144*9)	// fix ITR 104914, long config line length issue
#else
...
#ifdef ALPHA_CUSTOMIZE
static void atalk_strip_section(char *line, char *section, int len){
	strncpy(section, line+1, len-2);	// -2 means '[' & ']'
	section[len-2] = '\0';
}
#endif
    ...
    char * value)
{
    ...
    if (len<1) {
        ...
        /* Section name */
#ifdef ALPHA_CUSTOMIZE
		atalk_strip_section(line, section, len);
#else
        sscanf(line, "[%[^]]", section);
#endif
