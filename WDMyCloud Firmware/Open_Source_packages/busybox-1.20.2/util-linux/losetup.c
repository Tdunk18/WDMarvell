...
#define MAX_LOOP_DEV 600
#include <linux/version.h>

#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,1,0)
#include "linux_loop.h"
#define SUPPORT_LOOP_CONTROL_INF   1
#endif

#if defined(SUPPORT_LOOP_CONTROL_INF)
int get_next_free_loop_dev(void)
{
	int devnr = -1;
	int ctl_fd;
	...
	/* LOOP_CTL_GET_FREE will create loop device if there is more free loop dev. */
...
int losetup_main(int argc UNUSED_PARAM, char **argv)
{
...
#if defined(ALPHA_CUSTOMIZE) && defined(SUPPORT_LOOP_CONTROL_INF)
	if(opt == OPT_f)
	{
		char dev[LOOP_NAMESIZE];
		int free_idx = -1;
		free_idx = get_next_free_loop_dev();
		if(free_idx >= 0)
		{
			sprintf(dev, LOOP_FORMAT, free_idx);
			puts(dev);
			return EXIT_SUCCESS;
		}
	}
	...
	while (1) {
		...
		if (!s) {
#ifdef ALPHA_CUSTOMIZE
			if (n > MAX_LOOP_DEV && errno && errno != ENXIO)
#else
