...
HAS_MYPHOTO=`echo $CFLAGS | grep 'PROJECT_FEATURE_MYPHOTO=1' >/dev/null 2>&1;echo $?`
HAS_MYMUSIC=`echo $CFLAGS | grep 'PROJECT_FEATURE_MYMUSIC=1' >/dev/null 2>&1;echo $?`
NEED_DBUS=0

if [ "$HAS_MYPHOTO" = "0" ] || [ "$HAS_MYMUSIC" = "0" ]; then
	NEED_DBUS=1
fi
