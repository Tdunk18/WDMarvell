...
#define cgicTempDir "/usr/local/upload"
...
static void SpecSymbol2BackSlash(char *outdata, char *indata);
#ifdef ALPHA
static cgiParseResultType HandleFileUpload(void);
#endif

#ifdef ALPHA
/* alpha.tim for Debug . */
int debug_note(const char *format , ...) {
        va_list args;
        FILE *fp ;
...
//20110630 roy
inline int cgi_system(char *cmd)
{
   pid_t pid;
...
static cgiParseResultType HandleFirmwareUpload()
{
	char buf[1024];
	long end;
	int cnt;
	...
	if ( cgiContentLength < 0x100000 )
		return cgiParseConfig;
		
	errno = 0; //debug
	
	/* XXX: Do we have lock mechanism to avoid multiple firmware upload at the same time ? */
	/* FIXME: save the file to /usr/local/upload/firmware.bin */
	//FILE *ufp = fopen("/home/sddnd/public_html/cgi-bin/cgic205/firmware.bin", "w+");
    FILE *ufp = fopen("/usr/local/upload/nas_new_firmware", "w+");

	/* skip first 8 lines */
	/* get first line for the length of seperate . */
	fgets(buf, 1024, cgiIn);
	s_line_number = strlen(buf);
		
	for (cnt = 0; cnt < 7; cnt++) {
		fgets(buf, 1024, cgiIn);
		if ( cnt == 3 )
		{
			if ( strstr(buf,"cgi_restore_conf") != 0 )
				upload_type = 1;
		}
		//debug_note("%s",buf);
	}

	while ((cnt = fread(buf, 1, 1024, cgiIn)) > 0) {
		fwrite(buf, 1, cnt, ufp);
	}
	s_line_number += 2 ;
	s_line_number = 0 - s_line_number ;
	end = fseek(ufp, s_line_number, SEEK_END);
	
	/* Find last \r\n */
	do {
		fread(byte, 1, 1, ufp);
		fseek(ufp, -2, SEEK_CUR);
	} while (byte[0] != '\n');

	fread(byte, 1, 1, ufp);
	if (byte[0] == '\r')
		fseek(ufp, -1, SEEK_CUR);
	
	end = ftell(ufp);

	ftruncate(fileno(ufp), end);
	fclose(ufp);

	/* fill our cgiFormEntries for cgi_firmware_upload in system_mgr */
	cgiFormEntry *a, *b = NULL;

	b = (cgiFormEntry *)malloc(sizeof(cgiFormEntry));
	if (!b)
		return cgiParseMemory;
	memset(b, 0, sizeof(cgiFormEntry));
	b->attr = strdup("file");
	b->value = strdup("");
	b->valueLength = end;
	b->fileName = strdup("nas-new-firmware");
	b->contentType = strdup("");
	b->tfileName = strdup("/usr/local/upload/nas_new_firmware");
	b->next = NULL;

	a = (cgiFormEntry *)malloc(sizeof(cgiFormEntry));
	if (!a)
		return cgiParseMemory;
	memset(a, 0, sizeof(cgiFormEntry));
	a->attr = strdup("cmd");
	if ( upload_type == 0 )
	{
		a->value = strdup("cgi_firmware_upload");
		a->valueLength = strlen("cgi_firmware_upload");
	}	
	else
	{	
		a->value = strdup("cgi_restore_conf");
		a->valueLength = strlen("cgi_restore_conf");
	}
	a->fileName = strdup("");
	a->contentType = strdup("");
	a->tfileName = strdup("");
	a->next = b;
	cgiFormEntryFirst = a;
	...
	/*
	if (errno)
		debug_note("errno=%d - %s\n", errno, strerror(errno));
	else
		debug_note("seems good\n");
	*/
...
int main(int argc, char *argv[]) {
	int result;
#ifdef ALPHA
	int cgi_status; /*alpha.jack.20100913*/
#endif
	...
	cgiGetenv(&cgiAcceptCharset, "HTTP_ACCEPT_CHARSET");
	cgiGetenv(&cgiAcceptEncoding, "HTTP_ACCEPT_ENCODING");
	cgiGetenv(&cgiAcceptLanguage, "HTTP_ACCEPT_LANGUAGE");
	...
	if (cgiStrEqNc(cgiRequestMethod, "post")) {
		...
		if (cgiStrEqNc(cgiContentType, "application/x-www-form-urlencoded")) {	
...
#endif /* CGICDEBUG */
#ifdef ALPHA
			/* alpha.sddnd We change this part of implement to speed up firmware upload */
			/* Let's use the function HandleFirmwareUpload . */
			if (strstr(cgiScriptName, "system_mgr.cgi")) {
				/*alpha.jack.20100913+*/
				cgi_status = HandleFirmwareUpload();
			    if ( cgi_status != cgiParseSuccess ) {			        	
		        	if ( cgi_status == cgiParseConfig ) {
			if (cgiParsePostMultipartInput() != cgiParseSuccess) {
				...
				/*alpha.jack.20100913 end*/
			} /* End of firmware upload. */
#ifdef PROJECT_FEATURE_CUSTOM_WD
			else if (strstr(cgiScriptName , "webfile_mgr.cgi") ||
					 strstr(cgiScriptName , "p2p_upload.cgi" ))     {
#else
			else if (strstr(cgiScriptName , "webfile_mgr.cgi"))	{
#endif
				cgi_status = HandleFileUpload();
				if (cgi_status != cgiParseSuccess) {
					cgiFreeResources();
					return -1 ;
				}
			}
			else if (cgiParsePostMultipartInput() != cgiParseSuccess) {
#else
			if (strstr(cgiScriptName , "webfile_mgr.cgi"))	{
				cgi_status = HandleFileUpload();
				if (cgi_status != cgiParseSuccess) {
					cgiFreeResources();
					return -1 ;
	...
	} else if (cgiStrEqNc(cgiRequestMethod, "put")) { /* Temp Hack to allow PUT handling */
		if (cgiStrEqNc(cgiContentType, "application/x-www-form-urlencoded")) {
			if (cgiParsePostFormInput() != cgiParseSuccess) {
				cgiFreeResources();
				return -1;
		...
		putfp = fopen("/tmp/putxxx.txt", "w");

		xfp = popen("env", "r");
		fputs(">>>----- (DEBUG) env ------\n", putfp);
		while (fgets(buf, 1024, xfp)) {
			fputs(buf, putfp);
		}
		pclose(xfp);
		fputs(">>>----- (DEBUG) env end------\n\n", putfp);
...
static void cgiFreeResources() {
	...
	if (cgiRestored) {
		...
		free(cgiContentType);
		free(cgiAccept);
		free(cgiAcceptCharset);
		free(cgiAcceptEncoding);
		free(cgiAcceptLanguage);
		free(cgiUserAgent);
		free(cgiReferrer);
	}
...
//amy.alpha 2009.6.15++
void msg_debug(fmt, args)
{
	FILE *fp;
	char	buf[2560];
	fp = fopen("/tmp/cgi", "a+");
	sprintf(buf,(const char *)fmt,args);
	fputs(buf, fp);
	fclose(fp);	
}


cgiFormResultType cgiFormFileRename(
	char *name, cgiFilePtr *cfpp, char *rename)
{
	cgiFormEntry *e;
//	cgiFilePtr cfp;
	char filename[1024],re_filename[1024];
	char buf[2048];
	
	memset(filename,0,sizeof(filename));
	memset(re_filename,0,sizeof(re_filename));
	...
	//+alpha.eve.20090730:support file name include:' ','#','$','%','^','&','(',')','+','[',']',';','\\','\'
	SpecSymbol2BackSlash(filename,e->tfileName);
	SpecSymbol2BackSlash(re_filename,rename);
	
	//sprintf(buf,"mv %s %s",e->tfileName,rename);	//-alpha.eve.20090730
//#ifdef ALPHA	
    //sprintf(buf,"mv \"%s/%s\" \"%s\"",cgicTempDir,filename,re_filename);	//+alpha.eve.20090730:support file name include:' ','#','$','%','^','&','(',')','+','[',']',';','\\','\'
//#else
    //sprintf(buf,"mv \"%s\" \"%s\"",filename,re_filename);	//+alpha.eve.20090730:support file name include:' ','#','$','%','^','&','(',')','+','[',']',';','\\','\'
    sprintf(buf,"mv %s %s",filename,re_filename);	//+alpha.eve.20090730:support file name include:' ','#','$','%','^','&','(',')','+','[',']',';','\\','\'
//#endif
	system(buf);
...
cgiFormResultType cgiFormFileRename_wfs(
	char *name, cgiFilePtr *cfpp, char *rename)
{
	cgiFormEntry *e;
//	cgiFilePtr cfp;
	...
	sprintf(buf,"mv \"%s\" \"%s\"",e->tfileName,rename);		
	system(buf);
...
cgiEnvironmentResultType cgiWriteEnvironment(char *filename) {
	...
	if (!cgiWriteString(out, cgiAcceptCharset)) {
		goto error;
	}
	if (!cgiWriteString(out, cgiAcceptEncoding)) {
		goto error;
	}
	if (!cgiWriteString(out, cgiAcceptLanguage)) {
		goto error;
	}
...
cgiEnvironmentResultType cgiReadEnvironment(char *filename) {
	...
	if (!cgiReadString(in, &cgiAcceptCharset)) {
		goto error;
	}
	if (!cgiReadString(in, &cgiAcceptEncoding)) {
		goto error;
	}
	if (!cgiReadString(in, &cgiAcceptLanguage)) {
		goto error;
	}
...
void SpecSymbol2BackSlash(char *outdata, char *indata)
{	
	int i=0;
	int inlen;
	char* out;
	
	if(	strchr(indata,' ') == NULL &&
		strchr(indata,'#') == NULL &&
		strchr(indata,'$') == NULL &&
		strchr(indata,'%') == NULL &&
		strchr(indata,'^') == NULL &&
		...
		strchr(indata,'\\') == NULL &&
		strchr(indata,'\'') == NULL &&
		strchr(indata,'{') == NULL &&			//add by eve on 20100407
		strchr(indata,'}') == NULL 				//add by eve on 20100407
		)
	{
	...
	for (i=0; i < inlen; i++)
	{
		...
		else if (indata[i] == '\{')	//support '\{',add by eve on 20100407
		{
			memcpy(out, "\\{", 2);
			out = out + 2;
		}
		else if (indata[i] == 0x7D)	//support '\}' -> add by eve on 20100407
		{
			memcpy(out, "\\}", 2);
			out = out + 2;
		}
		else if (indata[i] == 0x3D)	//support '\=' -> add by eve on 20100407
		{
			memcpy(out, "\\=", 2);
			out = out + 2;
		}
		else if (indata[i] == 0x60)	//support '`' -> add by eve on 20100407
		{
			memcpy(out, "\\`", 2);
			out = out + 2;
		}
...
static cgiParseResultType HandleFileUpload()
{
	char buf[1024],sep_buf[1024];
	long end;
	int cnt;
	char byte[1];
	int s_line_number = 0 ;
	int s_count = 0 ;
	char up_path[1024],org_filename[1024],cctype[256];
	char fvalue[256],abadon_tmp[1024];
	...
	/* skip first 8 lines */
	/* get first line for the length of seperate . */
	fgets(sep_buf, 1024, cgiIn);
	s_line_number = strlen(sep_buf);
	...
	for (;;) {
		...
		if ((buf[0] == '\r' && buf[1] == '\n') || buf[0] == '\n' ) {
			fgets(buf , 1024 , cgiIn);
			memset(up_path , 0 , sizeof(up_path));
			if (buf[strlen(buf)-2] == '\r' && buf[strlen(buf)-1] == '\n')
				strncpy(up_path , buf , strlen(buf)-2);
			else if (buf[strlen(buf)-1] == '\n')
				strncpy(up_path , buf , strlen(buf)-1);
		}
	}
	...
	for (;;) {
		...
		if (strstr(buf , "Content-Disposition:")) {
			...
			argNames[1] = "filename";
			argNames[2] = 0;
			argValues[0] = abadon_tmp;
			argValues[1] = org_filename;
			decomposeValue(buf, 
				fvalue, sizeof(fvalue),
				argNames,
				argValues,
    ...
    FILE *ufp = fopen("/usr/local/upload/fileToUpload", "w+");
	...
	/* fill our cgiFormEntries for cgi_firmware_upload in system_mgr */
	cgiFormEntry *a, *b, *c = NULL;
	
	/* struct C . */
	c = (cgiFormEntry *)malloc(sizeof(cgiFormEntry));
	if (!c)
		return cgiParseMemory;
	memset(c, 0, sizeof(cgiFormEntry));
	c->attr = strdup("fileToUpload");
	c->value = strdup("/usr/local/upload/fileToUpload");
	c->valueLength = end;
	c->fileName = strdup(org_filename); /* filename: */
	c->contentType = strdup(cctype); /* Content-Type: */
	c->tfileName = strdup("/usr/local/upload/fileToUpload");
	c->next = NULL;
	...
	memset(b, 0, sizeof(cgiFormEntry));
	b->attr = strdup("path");
	b->value = strdup(up_path); /* Upload path. */
	b->valueLength = strlen(up_path); /* Upload path length. */
	...
	memset(a, 0, sizeof(cgiFormEntry));
	a->attr = strdup("cmd");
	a->value = strdup("cgi_upload");
	a->valueLength = strlen("cgi_upload");
