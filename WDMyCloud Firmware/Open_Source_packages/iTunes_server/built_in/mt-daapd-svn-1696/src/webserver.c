...
void *ws_mainthread(void *arg) {
    ...
    ws_dprintf(L_WS_INF,"Server thread ws_mainthread() Created..%d\n",getpid());
    while(1) {
        ...
        ws_dprintf(L_WS_INF,"Starting ws_dispatcher() thread\n");
        if((err=pthread_create(&tid,NULL,ws_dispatcher,(void*)pwsc))) {
            ...
            ws_dprintf(L_WS_INF,"ws_dipatcher tid:%d \n",tid);
            pthread_detach(tid);
        }
        ws_unlock_unsafe();
    }
...
void *ws_dispatcher(void *arg) {
    ...
    WS_ENTER()
ws_dprintf(L_WS_INF,"Dispatcher thread ws_dispatcher() Created..%d\n",getpid());
    ...
    while(!connection_done) {
        ...
        ws_dprintf(L_WS_DBG,"Thread %d: got request\n",pwsc->threadno);
        ...
        if(handler == -1) {
            ...
            /* curtis@alpha 06_08_2009
            When webserver receives the packet which is transmitted by iTunes client,
            the webserver must request iTunes client to return MUSIC PASSWORD to it */

            //printf("webserver.c: ws_disparcher(): uri: %s\n",pwsc->uri);
            //printf("webserver.c: ws_disparcher(): hostname: %s\n",pwsc->hostname);

            iTunes_flag = strncmp("/content-codes", pwsc->uri, 12);
            //printf("webserver.c: ws_disparcher(): iTunes_flag=%d\n",iTunes_flag);
						//printf("webserver.c: ws_disparcher(): auth_status=%d\n",auth_status);
						printf("uri is %s\n",pwsc->uri);

//         		if(iTunes_flag==0) { /* connecting webserver from webpage */
         		if(strlen(pwsc->uri)==1 || strstr(pwsc->uri, "html")) { /*add by alpha ibuki */
         			char *html_key=NULL;
         			html_key=strstr(pwsc->uri, "html");


         			if(auth_status ==0 ){ /* Unauthorized */
         				if(html_key!=NULL) { /* check html_key */
         					html_key=strstr(pwsc->uri, "alpha_firefly");

										if(html_key!=NULL) {/* html_key is correct */
											can_dispatch=1;
											auth_status=1;
											auth_ip=strdup(pwsc->hostname);
										}
										else {
											connection_done=1;
         				...
         				else /* not html file, css, js, etc... */
       						can_dispatch=1;
       				}
       				else { /* Authorized  */
       					if(html_key!=NULL) {
       						int auth_ip_flag=0;
       						auth_ip_flag=strcmp(pwsc->hostname, auth_ip);
       						printf("webserver.c: ws_disparcher(): auth_ip_flag=%d\n",auth_ip_flag);

       						if(auth_ip_flag == 0)
       							can_dispatch=1;
       						else { /* hostname is changed */
       							html_key=strstr(pwsc->uri, "alpha_firefly");
       							if(html_key==NULL) {/* have no key */
       								connection_done=1;
											ws_returnerror(pwsc,401,"Unauthorized");
										}
										else {
...
void ws_defaulthandler(WS_PRIVATE *pwsp, WS_CONNINFO *pwsc) {
    ...
    //printf("webserver.c: ws_defaulthandler: ws_copyfile\n");
