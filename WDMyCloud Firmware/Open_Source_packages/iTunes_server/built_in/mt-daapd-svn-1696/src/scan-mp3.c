...
/* for id3tag release 03_31_2009 curtis@alpha */
#include "global.h" 
#include "debug.h"
...
/*
*unicode: 2 bytes
*curtis@alpha 03_31_2009
*/
int unicode_test(id3_ucs4_t const *native_text)
{
	int id3_ucs4_len = id3_ucs4_length(native_text);
	int index = 0;
	for(index = 0 ; index < id3_ucs4_len ; index++) {
#if 1
        // It's safe to convert from iso8859-1 to utf-8 BUT not for codepage except iso8859-1(Ex:Big5/GB2312...)
        if(native_text[index] > 127)
#else
		if(native_text[index] > 255)
#endif
			return 1;
	}
...
/* ucs4 format: long: [0XFF] [0XFF] [0XFF] [0XFF]
* convert usc4 to char[0]=[0XFF], char[1]=[0XFF], char[2]=[0XFF], char[3]=[0XFF]	              
* curtis@alpha 03_26_2009
*/
char *convert_ucs4_to_byte(id3_ucs4_t const *ucs_4)
{
	id3_length_t ucs4_len = 0;
	int uc4_num = 0;
	char *utf8;
	ucs4_len = id3_ucs4_length(ucs_4);
	DBG_FILE_SCAN(printf("scan-mp3.c: convert_ucs4_to_byte: ucs4_len = %d\n",ucs4_len);)
  utf8 = (char *)malloc(ucs4_len * sizeof(char) + 1);
  utf8[ucs4_len] = NULL ;
  if (utf8){
  	for(uc4_num = 0; uc4_num < ucs4_len; uc4_num++)	{
		utf8[uc4_num]=(char *)ucs_4[uc4_num];
		DBG_FILE_SCAN(printf("utf8[%d]=%X ",uc4_num,utf8[uc4_num]);)
		}
		DBG_FILE_SCAN(printf("\n");)
	}
  return utf8;
}
        ...
        if(have_text) {
            ...
            if(native_text) {
                ...
                if(conf_get_int("general","latin1_tags",0)) {
                	...
                	DBG_FILE_SCAN(printf("Latin\n");)

                } 
                else {
                		/* switch encode type 03_31_2009 curtis@alpha */                		
                		if (unicode_test(native_text)) {
											utf8_text=(char *)id3_ucs4_utf8duplicate(native_text);//unicode
											unicode16_text=1;
									
                			DBG_FILE_SCAN(printf("scan-mp3.c: scan_get_mp3info(): native_text=unicode\n");)
                		}
                		else {
											utf8_text = (char *)convert_ucs4_to_byte(native_text);//other codepage 
											unicode16_text=0;
								
                		  DBG_FILE_SCAN(printf("scan-mp3.c: scan_get_mp3info(): native_text=other codepage\n");)
                		}
                }
