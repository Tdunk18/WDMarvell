...
xbuild()
{
	...
	if [ ! -e $(readlink -f $PWD/../_xinstall/${PROJECT_NAME})/lib/libncurses.so.5 ]; then
	...
	./configure --host=${TARGET_HOST} --with-curses --prefix=$(readlink -f $PWD/../_xinstall/${PROJECT_NAME})
