...
DBPATH_MTP_DEV_ENTRY_DEVPATH="${DBPATH_MTP_DEV_ENTRY}/devpath"
...
insert_mtp_to_usbdev_db () {
	...
	#devpath
	xmlpath=$(printf $DBPATH_MTP_DEV_ENTRY_DEVPATH $idx)
	xmldbc -S $MISC_USBINFO_SOCK_PATH -s $xmlpath "$dev_sys_path"
}
...
remove_ups_from_usbdev_db () {
	...
	while [ $idx -le $xmldb_ups_cnt ]; do
		xmlpath=$(printf $DBPATH_UPS_DEV_ENTRY_DEV $idx)
		#echo "xmlpath="$xmlpath > /dev/console
		db_dev=$(xmldbc -S $MISC_USBINFO_SOCK_PATH -g $xmlpath)
		#echo "db_dev="$db_dev > /dev/console
...
#echo "do_printer_ups.sh" > /dev/console
...
renew)
	...
	for uevent in /sys/bus/usb/devices/*-*:*\.*/uevent ; do
		...
		#echo "DEV_BUSNUM="$DEV_BUSNUM > /dev/console
		...
		USB_DEV=$(echo $uevent | awk -F '/' '{print $(NF-1)}')
		...
		if [ -e /tmp/upsin ]; then	
			...
			for uevent in /sys/bus/usb/devices/*-*:*\.*/uevent ; do
				...
				if [ "$DRIVER" = "usbfs" ]; then
					...
					#echo "usb_devpath="$usb_devpath > /dev/console
					usb_dev_sub_path=$(echo $usb_devpath | awk 'BEGIN{FS="/"; OFS="/"}{print $6}')
					USB_DEV=$(ls $usb_devpath | grep "$usb_dev_sub_path:*")
					#echo USB_DEV=$USB_DEV USB_LOGI_PORT=$USB_LOGI_PORT > /dev/console
...
add)
	...
	dev_sys_dev_path="$dev_sys_path/.."
	
	# check busnum file exist
	if [ ! -e $dev_sys_dev_path/busnum ]; then                 
		exit 0                            
	fi    
	...
	#echo "PRODUCT="$PRODUCT > /dev/console
	...
	#if [ $vendordef_in = 1 ]; then
	if [ $vendordef_in = 1  -a $image_in = 0 ]; then
	#	echo -e "USB_PHY_PORT = $USB_PHY_PORT" > /dev/console
		#Try to apply the DC .
		mtp_backup -c 
		PPPING=$?
		#echo "mtp_backup -c -D > /dev/console" > /dev/console
	...
	if [ -e /tmp/upsin ]; then		# check UPS device exist
    ups_info
