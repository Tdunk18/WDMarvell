...
class StringTableReader {
    ...
    const STRINGS_DIR_WINDOWS = 'C:\\SVNBranches\\Nas3G\\etc\\nas\\strings';
    const STRINGS_DIR_LINUX = '/etc/nas/strings';

    protected static $stDirPath;
    protected $stFileName;
    protected $locale;
    protected $locales; //supported locales
    protected $stringTable = array();

    /**
     * fn to read locale directories and add directory names to locales array
     */
    protected function _readLocales() {
        $this->locales = array();
        if (($dh = opendir(self::$stDirPath)) == true ) {
     ...
     *
     */
    protected function _readStringTable() {
    ...
    public function __construct($locale, $fileName) {
        if ((isset($_SERVER["WINDIR"]) && (stripos($_SERVER["WINDIR"], "windows") !== false)) ||
            ...
            self::$stDirPath = self::STRINGS_DIR_WINDOWS;
        } else {
            self::$stDirPath = self::STRINGS_DIR_LINUX;
        }
