	...
	if(gnChecked==0||isSamePath(gaProj[gnChecked].sPPath,aRProj[0].sPPath))
	{
		if(gnChecked!=0)
		{
			if(aRProj[0].sLangId!=gaProj[0].sLangId)
				alert("Объединенная справочная система "+aRProj[0].sPPath+" использует язык, отличный от главной системы; в связи с этим алфавитный указатель и полнотекстовый поиск не будут доступны в объединенной справочной системе.");
		}
		...
		alert("Загрузка не удалась; нажмите «Обновить».");
}
