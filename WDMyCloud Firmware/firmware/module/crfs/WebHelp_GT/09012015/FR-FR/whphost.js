...
function whCom(sName,sComFile)
{
	...
	this.getDivHTML=function()
	{
		var sHTML="";
		if(gbMac&&gbIE4)
			sHTML+="<DIV ID="+this.msDivId+" ALIGN=" +alignment+" STYLE=\"position:absolute;z-index:1;" +alignment+" :0;top:0;width:100%;height:100%;margin:0;padding:0;border:0;\">";
		else if(gbIE5)
			sHTML+="<DIV ID="+this.msDivId+" ALIGN=" +alignment+" STYLE=\"position:absolute;z-index:1;" +alignment+" :0;top:0;width:100%;height:100%;\">";
		else if(gbIE4||gbWindows)
			sHTML+="<DIV ID="+this.msDivId+" ALIGN=" +alignment+" STYLE=\"position:absolute;z-index:1;" +alignment+" :0;top:0;width:100%;height:100%;visibility:hidden\">";
		else if(gbMac && (gbNav6 || gbChrome))
			sHTML+="<DIV ID="+this.msDivId+" ALIGN=" +alignment+" STYLE=\"position:absolute;z-index:1;" +alignment+" :0;top:0;width:100%;height:100%;visibility:hidden\">";
		else if(gbUnixOS)
			sHTML+="<DIV ID="+this.msDivId+" ALIGN=" +alignment+" STYLE=\"position:absolute;z-index:1;" +alignment+" :0;top:0;width:100%;height:100%;visibility:hidden\">";
		else
			sHTML+="<DIV ID="+this.msDivId+" ALIGN=" +alignment+" STYLE=\"position:absolute;z-index:1;" +alignment+" :0;top:0;width:100%;height:"+parent.height+";visibility:hidden\">";
		sHTML+="</DIV>";
		return sHTML;
	}
}  
	...
	if(gnChecked==0||isSamePath(gaProj[gnChecked].sPPath,aRProj[0].sPPath))
	{
		if(gnChecked!=0)
		{
			if(aRProj[0].sLangId!=gaProj[0].sLangId)
				alert("Le système d’aide fusionné "+aRProj[0].sPPath+" utilise une langue différente du système d’aide principal. Par conséquent, les fonctionnalités de recherche en texte intégral et de recherche d’index seront désactivées dans le système d’aide fusionné.");
		}
		...
		alert("Impossible de charger correctement l’élément, veuillez cliquer sur Actualiser.");
}
