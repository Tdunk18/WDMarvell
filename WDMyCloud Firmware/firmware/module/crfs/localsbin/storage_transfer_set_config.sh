...
while [ "$1" != "" ]; do
	case $1 in
			--auto_transfer )       shift
                              if [ "$1" == "true" ]; then
                                sed -i 's/enable =.*/enable = true/' /usr/local/config/StorageSet.ini
                              elif [ "$1" == "false" ]; then
                                sed -i 's/enable =.*/enable = false/' /usr/local/config/StorageSet.ini                             
                              else
                                exit 1
                              fi                              
                              ;;
			--transfer_mode )       shift
                              if [ "$1" == "move" ] || [ "$1" == "copy" ]; then
                                sed -i 's/mode =.*/mode = '${1}'/' /usr/local/config/StorageSet.ini
                              else
                                exit 1
                              fi                              
                              ;;
