...
expireAlert=`sqlite3 "${alertdb}" "SELECT id FROM AlertHistory WHERE log_time < datetime(${removetime}, 'unixepoch')"`
for i in $expireAlert; do
	#fireAlert -a $i -x
...
if [ $count -gt $numskeep ]; then
	...
	exceedAlert=`sqlite3 "${alertdb}" "SELECT id FROM AlertHistory WHERE id <= (SELECT id FROM AlertHistory ORDER BY id DESC LIMIT 1 OFFSET ${numskeep})"`
