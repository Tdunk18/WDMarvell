...
if ([ "$backupEnabled" = "true" ] && [ ! -z "$backupShare" ] && [ -d "/shares/$backupShare/TimeMachine" ]); then
    echo "$hostname-backup" >> $CONF_FILE
    echo "_adisk._tcp. local." >> $CONF_FILE
    echo "548" >> $CONF_FILE
    echo "dk0=adVN=TimeMachine,adVF=0x81" >> $CONF_FILE
    echo "" >> $CONF_FILE
fi
