...
function do_login($username)
{
	...
	if ($username != "")
	{
		...
		if ($timeout_val != -1 & $timeout_val <= $web_timeout) //Already login
		{
			$ret = 1;
			$GLOBALS['_url'] = "/web/home.php";
			$userinfo = posix_getpwnam($username);
			$adAdmin=0;
			//echo sprintf("alert('2%s');", $username);
			if (strstr($username, "\\"))
			{
				//echo sprintf("alert('3%s');", $username);
				//exec("wbinfo -n \"$username\"",$ret);

				$fp = popen("wbinfo -n \"$username\"", "r");
				$line = fgets($fp, 512);
				$line = trim($line);
				...
				if(strlen($line)!=0 && substr($line, 0, 9) == "S-1-5-21-")
				{
					list($sid) = sscanf($line, "%s SID_USER");
					//echo sprintf("alert('%s');", $sid);
					
					//wbinfo --user-domgroups S-1-5-21-457946843-2673880060-1604608496-500
					$fp = popen("wbinfo --user-domgroups $sid", "r"); 
			...
			if($userinfo['uid']==500 || $userinfo['gid']==1001 || $adAdmin==1)	//fish20131119+ Because the 'admin' account name can be modify by end user
			{
				$_SESSION['isAdmin'] = 1;
				setcookie('isAdmin', 1);
				$GLOBALS['user_login'] = 0;
				//fish20130709+ for eula page
				if (!file_exists('/etc/.eula_accepted')) {
					$GLOBALS['_url'] = "/web/eula.php";
				}
			}
			else
			{
				$_SESSION['isAdmin'] = 0;
				setcookie('isAdmin', 0);
				$GLOBALS['user_login'] = 1;
				$GLOBALS['_url'] = "/web/myhome.php";
			}
		}
	}
	else
...
if ( $_POST['action'] != "")
{
	...
	if ( $_POST['action'] == "login" && isset($_POST['username']) && $_POST['username'] != "")
	{
	...
	$r = new stdClass();
	$r->success = true;
	//$r->isAdmin = $_COOKIE['isAdmin'];
	//$r->username = $_COOKIE['username'];
	echo json_encode($r);
	exit;
}
...
<link rel="stylesheet" type="text/css" href="/web/css/style.css?v=WDV1.05">
...
<script language="javascript" src="/web/function/define.js?id=<?=$r; ?>"></script>
<script language="javascript" src="/web/function/diag.js?id=<?=$r; ?>"></script>
<script language="javascript" src="/web/function/function.js?id=<?=$r; ?>"></script>
<script language="javascript" src="/web/function/init.js?id=<?=$r; ?>"></script>
<script language="javascript" src="/web/function/button.js?id=<?=$r; ?>"></script>
<script language="javascript" src="/web/function/raid_define.js?id=<?=$r; ?>"></script>
<script language="javascript" src="/web/function/raid_diag.js?id=<?=$r; ?>"></script>
<script language="javascript" src="/web/function/homeDiag.js?id=<?=$r; ?>"></script>
<script language="javascript" src="/web/function/home.js?id=<?=$r; ?>"></script>
<script language="javascript" src="/web/function/sub_menu.js?id=<?=$r; ?>"></script>
<script language="javascript" src="/web/function/home_usb.js?id=<?=$r; ?>"></script>
<script language="javascript" src="/web/function/fw.js?id=<?=$r; ?>"></script>
<script language="javascript" src="/web/function/input.js?id=<?=$r; ?>"></script>
<script language="javascript" src="/web/function/device_active.js?id=<?=$r; ?>"></script>
<script language="javascript" src="/web/function/eula.js?id=<?=$r; ?>"></script>
<script language="javascript" src="/web/function/rest.js?id=<?=$r; ?>"></script>
<script language="javascript" src="/web/function/cloud.js?id=<?=$r; ?>"></script>
<script language="javascript" src="/web/function/web_idle_timeout.js?id=<?=$r; ?>"></script>
<script language="javascript" src="/web/function/language.js?id=<?=$r; ?>"></script>
<script language="javascript" src="/web/function/rc4.js?id=<?=$r; ?>"></script>
<script language="javascript" src="/web/function/users.js?id=<?=$r; ?>"></script>
<script language="javascript" src="/web/function/quota.js?id=<?=$r; ?>"></script>
<script language="javascript" src="/web/function/system.js?id=<?=$r; ?>"></script>
<script type="text/javascript" src="/web/function/notification.js?id=<?=$r; ?>"></script>
<script type="text/javascript" src="/web/function/sms.js?id=<?=$r; ?>"></script>
<!-- Date Format -->
<script type="text/javascript" src="/web/jquery/date_format-1.2.3.js"></script>
<!--<script src="/web/lib/msg.php?v=<?=$r; ?>" type="text/javascript"></script>-->
...
<?php if (do_login($username) == 1) { /* Already login */ ?>
