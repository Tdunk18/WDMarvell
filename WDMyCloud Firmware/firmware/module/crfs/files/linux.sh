...
#
#echo "tsp_tunnel_interface: ($TSP_TUNNEL_INTERFACE)"        > /tmp/tsp.txt
#echo "tsp_client_address_ipv6: ($TSP_CLIENT_ADDRESS_IPV6)" >> /tmp/tsp.txt
#echo "tsp_prefix: ($TSP_PREFIX)"                           >> /tmp/tsp.txt
#echo "tsp_prefixlen: ($TSP_PREFIXLEN)"                     >> /tmp/tsp.txt

if [ "$TSP_TUNNEL_INTERFACE" == "tun0" ]; then
    LAN=lan0
elif [ "$TSP_TUNNEL_INTERFACE" == "tun1" ]; then
    LAN=lan1
fi

if [ "$LAN" != "" -a "$TSP_CLIENT_ADDRESS_IPV6" != "" ]; then
    xmldbc -s /network_mgr/$LAN/tunnel_broker/ipv6address $TSP_CLIENT_ADDRESS_IPV6

    xmldbc -D /etc/NAS_CFG/config.xml
    save_mtd  /etc/NAS_CFG/config.xml
    
    echo $TSP_CLIENT_ADDRESS_IPV6 > /tmp/$TSP_TUNNEL_INTERFACE.txt	# for UI
fi
