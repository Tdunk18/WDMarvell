...
#define LOAD_DEFAULT_RAMDISK_ADDR	0xE00000
int load_ramdisk_error_flag = 0;
extern int push_reset_button;
extern int kernel_error_flag;  //alpha.jack.20140221+
extern int load_kernel_count;
#endif
...
#if 1 //ALPHA_CUSTOMIZE
				//alpha.jack.20140221+				
				//printf("dbg: rd_addr 1=%x,load_kernel_count=%d,load_ramdisk_error_flag=%d\n",rd_addr,load_kernel_count,load_ramdisk_error_flag);
				if ( kernel_error_flag == 0 ) {
					rd_addr = simple_strtoul(argv[2], NULL, 16);
				} else {
					load_default_ramdisk:
					rd_addr = LOAD_DEFAULT_RAMDISK_ADDR;
					//jack debug  
					//printf("dbg: rd_addr 1=%x,load_kernel_count=%d,load_ramdisk_error_flag=%d\n",rd_addr,load_kernel_count,load_ramdisk_error_flag);
					if ( load_ramdisk_error_flag == 1 )
					{
						printf ("### Ramdisk error, Loading Rescure Firmware from NAND Flash ###\n");
						load_ramdisk_error_flag++;
						kernel_error_flag = 3;
						*((ulong  *)(0xF1018148)) = 0x00100800; /* detect ramdisk broken and blinking HD led */
						run_command("nand read.e 0xa00000 0xC800800 0xf00000;bootm 0xa00000 0xe00000",0);
					}
				}
...
#if 1	//ALPHA_CUSTOMIZE
//jack debug    
//    printf("dbg:load_ramdisk_error_flag=%d\n",load_ramdisk_error_flag);
//    printf("dbg:rd_addr 2=%x\n",rd_addr);
#endif
...
#if 1	//ALPHA_CUSTOMIZE
			{
				if ( load_ramdisk_error_flag > TRY_ERROR_COUNT-1 )
					return 1;
				else
				{
					load_ramdisk_error_flag++;
					rd_addr = LOAD_DEFAULT_RAMDISK_ADDR;
					goto load_default_ramdisk;
				}   
			}
#else
				return 1;
...
#if 1	//ALPHA_CUSTOMIZE
			//alpha.jack.20140221+
			if ( load_ramdisk_error_flag < TRY_ERROR_COUNT )
			{
				load_ramdisk_error_flag++;
				rd_addr = LOAD_DEFAULT_RAMDISK_ADDR;
				goto load_default_ramdisk;
