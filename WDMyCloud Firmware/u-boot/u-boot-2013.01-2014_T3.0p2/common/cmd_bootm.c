...
#include <fs.h>
extern int find_usb_storage;
#define LOAD_DEFAULT_KERNEL_ADDR	0xa00000  //alpha.jack.20140221+
int kernel_error_flag=0;	//alpha.jack.20140221+
//int push_usb_button=0;
int push_reset_button=0;
int load_kernel_count=0;
#endif
...
#if 1	//ALPHA_CUSTOMIZE
	//Casper, GrandTeton no usb copy button, so, use reset button (mpp_50) detection.
	value32 = *((ulong  *)(0xF1018140));
	if ( ( value32 & (1<<(50-32)) ) == 0 ) {
		printf("boot_get_kernel : Reset button have been pushed\n");
		push_reset_button = 1;
		retry_count = 3;		
	} else {
		push_reset_button = 0;
...
load_default_kernel:
  //jack debug
  //printf("dbg: kernel_error_flag=%d\n",kernel_error_flag);
  //printf("dbg: load_kernel_count=%d\n",load_kernel_count);
  
	if ( push_reset_button == 0 )
	{
		if ( load_kernel_count == 1 )
		{
			//printf ("### Kernel error, Loading Rescure Firmware from NAND Flash  ###\n");
			kernel_error_flag = 1;
			run_command("nand read.e 0xa00000 0xC800800 0xF00000;",0);
			*((ulong  *)(0xF1018148)) = 0x00100800; /* detect ramdisk broken and blinking HD led */
		}
	}
	else
	{
		...
		if ( load_kernel_count == 1 )
		{
			if ( find_usb_storage == 0 )
	      	...
	      	else
	      	{
	        	...
	        	len_read = fs_read("/GrandTeton/u-boot.bin", 0x1000000, 0, 0);
	        	if ( len_read > 0 ) 
	        	{
	        		printf("#####################\n");
	          		printf("###               ###\n");
					...
					*((ulong  *)(0xF1018148)) = 0x00500000; /* blinking HD blue/Red led 1 for boot from USB */
	          		run_command("bubt /GrandTeton/u-boot.bin nand usb; reset;",0);
	        	}
	        	else
	          	printf("### Can not find u-boot file ###\n");
	      	}
		} 
		else if ( load_kernel_count == 2 )
		{
			printf ("### Loading Firmware from NAND Flash ###\n");
			run_command("nand read.e 0xa00000 0x500000 0x500000;nand read.e 0xf00000 0xa00000 0x500000;",0);
		}
		else if ( load_kernel_count == 3 )
		{
			printf ("### Kernel error, Loading Rescure Firmware from NAND Flash ###\n");
			kernel_error_flag = 1;
			run_command("nand read.e 0xa00000 0xC800800 0xF00000;",0);
...
//alpha.jack.20140221 end
	...
	*os_data = *os_len = 0;
	...
	case IMAGE_FORMAT_LEGACY:
		...
		if (!hdr)
		{
			if ( load_kernel_count > retry_count-1 )
				return NULL;
			else
			{
				load_kernel_count++;
				img_addr = LOAD_DEFAULT_KERNEL_ADDR;
				goto load_default_kernel;
			}
		}
		//alpha.jack.20140221 end
#else
		...
		default:
			...
			//ALPHA_CUSTOMIZE
			//alpha.jack.20140221+
			if ( load_kernel_count < retry_count )
			{
				load_kernel_count++;
				img_addr = LOAD_DEFAULT_KERNEL_ADDR;
				goto load_default_kernel;
